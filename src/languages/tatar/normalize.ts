...
export function normalizeTatar(input: string): string {
    ...
    s = rewrite(s, new RegExp(`(?<![\\d.,/])(\\d+)\\s?-\\s?(${SFX}{1,6})${NOT_LETTER_AFTER}`, "gu"),
        (whole, digits: string, sfx: string) => attach(whole, digits, sfx));
    s = rewrite(s, new RegExp(`(?<![\\d.,/])(\\d+)(${SFX}{1,6})${NOT_LETTER_AFTER}`, "gu"),
        (whole, digits: string, sfx: string) => attach(whole, digits, sfx));
    ...
    s = rewrite(s, /(\d)\s?=\s?(?=\d)/gu, "$1 тигез ");
    s = rewrite(s, /(\d)\s?×\s?(?=\d)/gu, "$1 тапкыр ");
    ...
    s = rewrite(s, new RegExp(`(\\d)\\s?°\\s?(\\d+)\\s?[′']\\s?(${SFX}{1,5})${NOT_LETTER_AFTER}`, "gu"),
    ...
    s = rewrite(s, /(\d)\s?°\s?(\d+)\s?[′']/gu, "$1 градус $2 минут ");
    ...
    s = rewrite(s, new RegExp(`(?<![\\d.,])(\\d+)\\s(${ORD_SUFFIX})${NOT_LETTER_AFTER}`, "gu"),
        (whole, digits: string, sfx: string) => attach(whole, digits, sfx));
