...
export function normalizeHausa(input: string): string {
    ...
    s = tr(s, /B\.?C\.?(?=\.?kafin)/giu, "");
    s = tr(s, /(?<![\p{L}\p{M}])B\.?C\.?E?\.?(?![\p{L}\p{M}])/giu, "kafin haihuwar Yesu");
    ...
    s = tr(s, /(?<![\p{L}\p{M}])(?:US|uS)\s?\$\s?(?=\d)/giu, "dala ");
    ...
    s = tr(s, /(?<![\d.,])(\d+\.\d+)\s*[-–]\s*(\d+\.\d+)(?![\d.])/gu, "$1 zuwa $2");
    ...
    s = tr(s, /(?<![\d.,])(\d(?:[\d,]*\d)?)\s*[-–]\s*(\d(?:[\d,]*\d)?)(?![\d.])/gu, "$1 zuwa $2");
    ...
    s = tr(s, /(?<![\d:,])([01]?\d|2[0-3]):([0-5]\d)(?![:.\d])(?:\s*([Aa]\.?[Mm]\.?|[Pp]\.?[Mm]\.?))?/giu,
    ...
    s = tr(s, /(?<![\d.,])(\d+)\.(\d+)\s?(km\/h|m\/s|km\/u|mph|mil\/awa|km|m|kg|mm|cm)(?![\p{L}\p{M}])/giu,
    ...
    s = tr(s, /(?<!\d)(\d+)\s?(km|m|kg|mm|cm)\s*\/\s*(h|s|u)(?![\p{L}\p{M}])/giu,
    ...
    s = tr(s, /(?<!\d)(\d+)\s?(mph|kph|mil\/awa)(?![\p{L}\p{M}])/giu,
        (m0, n: string, u: string) =>
            `${hausaNumber(Number(n))} ${u.toLowerCase() === "kph" ? "kilomita" : "mil"} a awa`);
    s = tr(s, /(?<!\d)(\d+)\s?Mbit\/s(?![\p{L}\p{M}])/giu, (m0, n: string) =>
    ...
    s = tr(s, /(?<![\p{L}\p{M}\p{Nd}])(km|kg|mm|cm)(?=\s\p{Nd})/gu,
    ...
    s = tr(s, /(?<![\p{L}\p{Nd}])[-−](\d+)(?!\s*[-\d])/gu, "rashin $1");
    ...
    s = tr(s, /\s&\s/gu, " da ");
    s = tr(s, /(\S)\s*=\s*(\S)/gu, "$1 daidai $2");
    s = tr(s, /(\d)\s*<\s*(\d)/gu, "$1 kasa da $2");
    s = tr(s, /(\d)\s*>\s*(\d)/gu, "$1 fiye da $2");
    s = tr(s, /(\d)\s*×\s*(\d)/gu, "$1 sau $2");
    ...
    s = tr(s, /(?<![kK]ashi\s+)([\p{L}\p{M}\d]+ maki [\p{L}\p{M} ]+?)\s*%\s*(?![\p{L}\p{M}])/gu, "kashi $1");
