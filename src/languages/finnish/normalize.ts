...
export function normalizeFinnish(input: string): string {
    ...
    t = rewrite(t, /(?<![\d.,:])(\d{1,2})\.(\d{1,2})\.(?!\d)/gu, (m, d: string, mo: string) =>
        isDay(Number(d)) && isMonth(Number(mo)) && ordinal(Number(d)) !== undefined
    ...
    t = rewrite(t, /(?<![\d.,:])(\d{1,3}):s(?![\p{L}\p{M}])/gu, (m, n: string) => ordinal(Number(n)) ?? m);
    ...
    t = rewrite(t, /(?<=[%²³]|°[CFcf])\s*:[a-zåäö]+(?![\p{L}\p{M}])/gu, "");
    t = rewrite(t, /(?<=\d\s?(?:kg|km|cm|mm|m))\s*:[a-zåäö]+(?![\p{L}\p{M}])/gu, "");
    ...
    t = rewrite(t, /(?<=[bcdfghjklmnpqrstvwxz])['’](?=[a-zåäö]{1,4}(?![\p{L}\p{M}]))/giu, "");
    ...
    t = rewrite(t, /(\d),(\d+)(?!\d)/gu, (_m, int: string, frac: string) =>
    ...
    t = rewrite(t, /(\d)\s*°\s*C(?![\p{L}\p{M}])/gui, "$1 astetta");
    t = rewrite(t, /(\d)\s*°\s*F(?![\p{L}\p{M}])/gui, "$1 astetta fahrenheitia");
    t = rewrite(t, /(\d)\s*°/gu, "$1 astetta");
    ...
    t = rewrite(t, /(?<![\p{L}\p{M}\d])\+(?=\d)/gu, "plus ");
    t = rewrite(t, /(?<=\d)\s*\+\s*(?=\d)/gu, " plus ");
    ...
    t = rewrite(rewrite(rewrite(t, /=/gu, " yhtä suuri kuin "), /</gu, " pienempi kuin "), />/gu, " suurempi kuin ");
    ...
    t = rewrite(rewrite(t, /&amp;/gu, "&"), /\s*[&＆]\s*/gu, " ja ");
