...
export function makeGujaratiNormalizer(numbers: NumbersDef): (text: string) => string {
    ...
    return (input: string): string => {
        ...
        s = rewrite(s, /[૦-૯]/gu, (d) => String(d.charCodeAt(0) - 0x0ae6));
        ...
        s = rewrite(s, /(?<![\p{L}\p{M}])ડ[ોૉ]\.(\s+)(?=[\p{L}])/gu, "ડૉક્ટર$1");
        ...
        s = rewrite(s, /(?<![\d.,:])(\d{1,2}):\s?([0-5]\d):\s?([0-5]\d)(?![\d.,:])/gu, "$1 $2 $3");
        ...
        s = rewrite(s, /(\S)\+\s?(?=\d)/gu, "$1 પ્લસ ");
        s = rewrite(s, /(^|\s)\+\s?(?=\d)/gu, "$1પ્લસ ");
        ...
        s = rewrite(s, /±/gu, " પ્લસ માઈનસ ");
        s = postposedSign(s, "<", "કરતાં ઓછું");
        s = postposedSign(s, ">", "કરતાં વધુ");
        s = postposedSign(s, "÷", "દ્વારા વિભાજીત");
        s = rewrite(s, /\s?=\s?/gu, " બરાબર ");

        s = rewrite(s, /(\d)\s?°\s?C(?![\p{L}\p{M}])/giu, "$1 ડિગ્રી સેલ્સિયસ");
        s = rewrite(s, /(\d)\s?°\s?F(?![\p{L}\p{M}])/giu, "$1 ડિગ્રી ફેરનહીટ");
        s = rewrite(s, /(\d)\s?°/gu, "$1 ડિગ્રી");
