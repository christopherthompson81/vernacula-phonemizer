...
export function normalizeZhuang(input: string): string {
    ...
    s = s.replace(BRACKET, stripGloss);
    ...
    s = s.replace(new RegExp(`(?<![\\p{L}\\p{M}\\d])(\\d+)${BCE}\\s?[-–—]\\s?(\\d+)${BCE}`, "gu"),
        "gunghyenz gonq $1 daengz gunghyenz gonq $2");
    s = s.replace(new RegExp(`(?<![\\p{L}\\p{M}\\d])(\\d+)${BCE}`, "gu"), "gunghyenz gonq $1");
    ...
    s = s.replace(/(?<![\d.,])(\d+(?:\.\d+)?)\s?%/gu, "bak faenh cih $1");
    ...
    s = s.replace(DATE_RANGE, " daengz ");
    s = s.replace(RANGE, (whole, a: string, b: string) => (Number(a) < Number(b) ? `${a} daengz ${b}` : whole));
