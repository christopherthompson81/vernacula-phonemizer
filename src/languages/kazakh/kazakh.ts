...
export function phonemizeWord(word: string): string {
  ...
  if (!VOWEL_OR_GLIDE.test(word.toLowerCase())) {
    ...
    return cons.map((c, i) => (i === cons.length - 1 ? `${c}ˈə` : `${c}ə`)).join(""); // stress the final ə
  }
  ...
  const isCons = (s: { ph: string; nucleus: boolean }): boolean => !s.nucleus && s.ph !== "w" && s.ph !== "j";
  if (segs.length >= 3 && isCons(segs[0]!) && isCons(segs[1]!) && isCons(segs[2]!)) {
...
class KazakhPhonemizer implements Phonemizer {
  text(input: string): string {
    ...
    for (const m of input.matchAll(TOKEN)) {
      ...
      if (m[1]) for (const part of m[1].split(/(?<=\p{Ll})(?=\p{Lu})/u)) emit(phonemizeWord(part));
      else if (m[2]) for (const ipa of numberToIpa(Number(m[2])).split(" ")) emit(ipa); // numbers are pre-phonemized
...
export function createKazakh(): Phonemizer { return new KazakhPhonemizer(); }
