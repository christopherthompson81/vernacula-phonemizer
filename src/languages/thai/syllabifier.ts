...
export function reorderThaiLeadingVowels(text: string): string {
    ...
    for (let i = 0; i < cs.length - 1; i++) {
        if (
            !(THAI_LEADING_VOWEL_RE.test(cs[i]!) && isThaiConsonant(cs[i + 1]!))
        )
            continue;
        ...
        if (
            ...
            THAI_LEADER_SONORANT.has(cs[i + 2] ?? "") &&
            cs[i + 3] === "อ"
        ) {
            const v = cs[i]!;
            cs[i] = cs[i + 1]!;
...
function thaiVowelSpan(
    s: readonly string[],
    i: number,
): { gs: string[]; glide?: "j" | "m" } | null {
    const c = s[i]!;
    ...
    if (c === "เ") {
        ...
        if (
            ...
            !(
                THAI_VSIGN.has(at2) ||
                THAI_LEADING_VOWEL_RE.test(at2) ||
                at2 === "อ"
            )
        ) {
        ...
        for (const pat of THAI_E_COMBOS) {
            if (s.slice(i, i + pat.length).join("") === pat)
                return { gs: [...pat] };
        }
    }
    if (c === "ั") {
    ...
    if (
        c === "อ" &&
        !THAI_O_GLOTTAL_NEXT.has(s[i + 1] ?? "") &&
        !(i === 0 && isThaiConsonant(s[i + 1] ?? ""))
    )
        return { gs: ["อ"] };
...
function thaiIsSilentLeader(units: readonly ThaiUnit[], i: number): boolean {
    const u = units[i];
    if (!u || u.kind !== "C") return false;
    const nx = units[i + 1];
    if (!nx || nx.kind !== "C") return false;
    return (
        (u.g === "ห" && THAI_H_RAISABLE.has(nx.g)) ||
        (i === 0 && u.g === "อ" && nx.g === "ย")
    );
}
...
function thaiSchwaFates(units: readonly ThaiUnit[]): Map<number, "o" | "a"> {
    ...
    if (
        seq.length >= 3 &&
        seq[seq.length - 1]!.sym === "ə" &&
        isC(seq[seq.length - 2]) &&
        isV(seq[seq.length - 3])
    )
        seq.pop();
    ...
    for (const t of seq)
        if (t.sym === "o" || t.sym === "a") out.set(t.owner, t.sym);
    return out;
}
 ...
 */
function rewriteThai<T>(
    seq: T[],
    width: number,
    apply: (window: (T | undefined)[]) => T[] | null,
): T[] {
    const out: T[] = [];
    for (let i = 0; i < seq.length; ) {
        const repl =
            i + width <= seq.length ? apply(seq.slice(i, i + width)) : null;
        if (repl) {
            out.push(...repl);
            i += width;
        } else {
 ...
 */
export function thaiPrep(
    reordered: string,
): {
    units: ThaiUnit[];
    fates: Map<number, "o" | "a">;
    ...
    for (const c of [...reordered]) {
        if (/[่-๋̄]/u.test(c)) {
            if (cleaned.length) markAt[cleaned.length - 1] = c as ThaiToneMark;
            continue;
        }
        if (c === "็") {
            if (cleaned.length) shortAt[cleaned.length - 1] = true;
            continue;
        }
        cleaned.push(c);
    }
    ...
    const shortByChar: boolean[] = [];
    {
        ...
        for (const wc of [...word]) {
            let m: ThaiToneMark | undefined;
            let sh = false;
            while (ci < cleaned.length && cleaned[ci] !== wc) {
                m = m ?? markAt[ci];
                sh = sh || !!shortAt[ci];
                ci++;
            }
            unitMarkByChar.push(markAt[ci] ?? m);
            shortByChar.push(!!shortAt[ci] || sh);
            ci++;
        }
    }
    ...
    const shortMark: boolean[] = [];
    {
        ...
        for (const u of units) {
            const len = u.kind === "C" ? 1 : u.gs.length;
            let m: ThaiToneMark | undefined;
            let sh = false;
            for (let k = 0; k < len; k++) {
                m = m ?? unitMarkByChar[p + k];
                sh = sh || !!shortByChar[p + k];
            }
            unitMark.push(m);
            shortMark.push(sh);
            p += len;
...
export function thaiScanSyllables(
    ...
    for (let i = 0; i < units.length; i++) {
        ...
        for (let j = onsetStart; j <= i; j++) {
            const uj = units[j]!;
            if (uj.kind === "C") onsetCs.push(uj.g);
        }
        if (onsetCs.length === 0) {
            onsetStart = i + 1;
            pendingLeaderClass = undefined;
            continue;
        ...
        if (
            onsetCs[0] === "ห" &&
            onsetCs.length >= 2 &&
            THAI_H_RAISABLE.has(onsetCs[1]!)
        )
            cls = thaiEffectiveClass(onsetCs[1]!, "ห");
        else if (
            onsetCs[0] === "อ" &&
            onsetCs.length >= 2 &&
            onsetCs[1] === "ย"
        )
            cls = thaiEffectiveClass("ย", "อ");
        else if (
            onsetCs.length === 1 &&
            pendingLeaderClass !== undefined &&
            THAI_H_RAISABLE.has(onsetCs[0]!)
        )
            cls = pendingLeaderClass; // implicit อักษรนำ
        else cls = thaiEffectiveClass(onsetCs[0]!, undefined);
        const long =
            u.kind === "V" && !shortMark[i]
                ? !THAI_SHORT_VSIGN.has(u.gs[u.gs.length - 1]!)
                : false;
        ...
        const live =
            glideVowel || THAI_LIVE_CODA.has(codaG) || (codaG === "" && long);
