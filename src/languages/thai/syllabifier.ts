...
/**
 * Burmese syllable boundaries: positions where a word may start/end. A Burmese syllable
 * begins at an ONSET base consonant (U+1000–U+1021) — i.e. one NOT stacked under the previous
 * (prev ≠ virama ◌္ U+1039) and NOT itself a coda (next ≠ asat ◌် U+103A, which would make it
 * end the current syllable). Medials / vowel signs / tone marks attach rightward, so they never
 * begin a word. Mirrors thaiTccBoundaries' role for segmentByDag. */
const MY_VIRAMA = 0x1039, MY_ASAT = 0x103a;

function burmeseSyllableBoundaries(cs: readonly string[]): Set<number> {
  const n = cs.length;
  const bounds = new Set<number>([0, n]);
  for (let i = 1; i < n; i++) {
    const c = cs[i]!.codePointAt(0)!;
    if (c < 0x1000 || c > 0x1021) continue; // base consonant / အ only
    const prev = cs[i - 1]!.codePointAt(0)!;
    const next = i + 1 < n ? cs[i + 1]!.codePointAt(0)! : 0;
    if (prev !== MY_VIRAMA && next !== MY_ASAT) bounds.add(i); // onset, not stacked, not a coda
  }
  return bounds;
}

/** Segment a Burmese run into words (DAG maximal-matching, FEWEST tokens) over `words`, with
 *  boundaries constrained to syllable starts so a word never splits mid-syllable. */
export function segmentBurmese(text: string, words: ReadonlySet<string>, maxLen: number): string[] {
  const cs = [...text];
  if (cs.length === 0) return [];
  return segmentByDag(cs, words, maxLen, burmeseSyllableBoundaries(cs));
}
