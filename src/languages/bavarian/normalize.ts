...
export function normalizeBavarian(input: string): string {
    ...
    for (const [re, word] of MULTI_DOT) s = rewrite(s, re, word);
    ...
    s = rewrite(s, ORD_RE, (whole, pre2: string | undefined, sp2: string | undefined,
    ...
    s = rewrite(s, new RegExp(`(?<![\\p{L}\\p{M}])(${ABBREV_ALT})\\.(\\s+)(?=[\\p{L}\\p{Nd}\\p{Sc}])`, "giu"),
    ...
    s = rewrite(s, /(?<=\p{Nd})(?<!(?<![\p{Nd}\.,])0)\.(?=\p{Nd}{3}(?!\p{Nd}))/gu, "");
    ...
    s = rewrite(rewrite(s, /℃/gu, "°C"), /℉/gu, "°F");
    s = rewrite(s, /(\p{Nd})\s*°\s*C(?![\p{L}\p{M}])/gui, "$1 Grad Celsius");
    s = rewrite(s, /(\p{Nd})\s*°\s*F(?![\p{L}\p{M}])/gui, "$1 Grad Fahrenheit");
    ...
    s = rewrite(s, /(\p{Nd})\s*°\s*[-–—](?=\p{L})/gu, "$1 Grad ");
    s = rewrite(s, /(\p{Nd})\s*°/gu, "$1 Grad");
    ...
    s = rewrite(s, new RegExp(`(^|[\\s(])[-−–]${DEGREE_AHEAD}`, "gu"), "$1minus ");
    s = rewrite(s, new RegExp(`(^|[\\s(])\\+${DEGREE_AHEAD}`, "gu"), "$1plus ");
    ...
    s = rewrite(s, /(?<!\p{Nd})(\p{Nd}{1,2})\/(\p{Nd})(?!\p{Nd})/gu, (m0, a: string, b: string) => {
    ...
    s = rewrite(s, /(?<![-–—\p{Nd}])(\p{Nd}{1,4})\s?[-–—]\s?(\p{Nd}{1,4})(?![-–—\p{Nd}])/gu,
