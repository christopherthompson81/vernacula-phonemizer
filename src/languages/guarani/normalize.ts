...
export function normalizeGuarani(input: string): string {
    ...
    s = rewrite(s, /(\d\s*)[ºª](\s*[CF](?![\p{L}\p{M}]))/gu, "$1°$2");
    ...
    s = rewrite(s, /(\d)\s*°\s*C(?![\p{L}\p{M}])/gui, "$1 Celsius");
    s = rewrite(s, /(\d)\s*°\s*F(?![\p{L}\p{M}])/gui, "$1 Fahrenheit");
    ...
    s = rewrite(s, /(?<![\d.,])(0)\.(\d{3})(?![\d.,])/gu, "$1,$2");
    s = rewrite(s, /(?<=\d)\.(?=\d{1,2}(?![\d.]))/gu, ",");
    ...
    s = rewrite(s, /(?<![\d.,–—-])(\d{4})\s?[-–—]\s?(\d{4})(?![\d–—-]|[.,]\d)/gu, "$1 guive $2 peve");
