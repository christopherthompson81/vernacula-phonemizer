...
export function normalizeSaraiki(input: string): string {
    ...
    s = rewrite(s, /(?<!\d)(?<![\d][.,،])([1-9]\d{0,2})((?:[,،]\s?\d{3})+)(?!\d)/gu,
    ...
    s = rewrite(s, /(\d)\s?°\s?C(?![\p{L}\p{M}])/gui, "$1 ڈگری سینٹی گریڈ");
    s = rewrite(s, /(\d)\s?°\s?F(?![\p{L}\p{M}])/gui, "$1 ڈگری فارن ہائیٹ");
    ...
    s = rewrite(s, /(^|(?<!\d)[\s(])[-−–]\s?(\d)/gu, "$1منفی $2");
