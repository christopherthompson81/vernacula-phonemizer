 ...
 *   · A DECIMAL WORD — Igbo borrows freely (pasent, dollar, naira), but it does NOT borrow "point": zero
 *     digit-point-digit instances. `ntụpọ` looked promising at 552 hits and means a SPOT or blemish
 *     (`ndị nwere ntụpọ`), never a decimal separator.
...
/** A decimal period. Not voiced — see the rule for why, and for why it must not simply be left alone either. */
...
export function normalizeIgbo(text: string): string {
    ...
    // ── 4. the decimal separator: not voiced, but not left as a PERIOD either ─────────────────────────────
    ...
    // pause is not.
    //
    // There is no word to voice it with. Igbo borrows its symbol vocabulary freely (pasent, dollar, naira) but does
    // NOT borrow "point": zero digit-point-digit instances. `ntụpọ` (552 hits) is a SPOT or blemish, never a
    // separator. So the FRACTION IS READ DIGIT BY DIGIT — what `sources.ts` prescribes for a "[NONE]
    // decimal-point" language — rather than as a cardinal: `3.14159` is "three one four one five nine", not
    // "three fourteen thousand one hundred and fifty-nine", which is what spacing the halves alone produced.
    s = s.replace(DECIMAL, (_m, whole: string, frac: string) => `${whole} ${[...frac].join(" ")}`);
