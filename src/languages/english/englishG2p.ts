...
const STOP_PIECE = new Set(["the", "and", "for", "are", "but", "not", "you", "all", "any", "can", "her",
  "was", "one", "our", "out", "his", "has", "had", "him", "how", "man", "new", "now", "old", "see", "two",
  "way", "who", "boy", "did", "its", "let", "put", "say", "she", "too", "use", "ate", "eat", "ion", "per"]);
const dropStress = (p: string): string => p.replace(/[0-2]$/, "");
const stressDown = (ph: string[]): string[] => ph.map((p) => p.replace(/1$/, "2"));
function collapseGeminates(ph: string[]): string[] {
  ...
  for (const p of ph) if (out[out.length - 1] !== p || VOWEL.has(dropStress(p))) out.push(p);
  return out;
}
...
function enforceSinglePrimary(ph: string[]): string[] {
  ...
  if (!seen) { const vi = out.findIndex((p) => VOWEL.has(dropStress(p))); if (vi >= 0) out[vi] = out[vi]!.replace(/[0-2]$/, "1"); }
  return out;
}
...
export function createEnglishG2p(model: EnglishG2pModel, dict: ReadonlyMap<string, string[]>, common: ReadonlySet<string>, arpabetToIpa: (phones: string[], word?: string) => string): EnglishG2p {
  ...
  function scoreTokAt(hist: string[], tok: string): [number, number] {
    for (let o = order - 1; o >= 0; o--) {
      ...
      if (e) { const hit = e.c.find(([t]) => t === tok); if (hit) return [Math.log(hit[1] / e.t) + (order - 1 - o) * Math.log(alpha), o]; }
    }
    return [Math.log(1e-7), -1];
  }
  function ngramDecode(w: string): string[] {
    let beam = [{ hist: [START, START, START, START], phones: [] as string[], score: 0 }];
    for (let i = 0; i < w.length; i++) {
      ...
      const filtered = sibLetter ? raw : raw.filter((ch) => { if (!ch) return true; const last = ch.split(" ").pop(); return last !== "S" && last !== "Z"; });
      ...
      for (const h of beam) {
        for (const chunk of chunks) {
          ...
          let s = h.score + lp;
          if (chunk === "" && emptyPenalized && ord < evpOrder) s -= evp;
          next.push({ hist: [...h.hist, `${c}:${chunk}`], phones: chunk ? [...h.phones, ...chunk.split(" ")] : h.phones, score: s });
        }
      }
  ...
  const allomorphS = (stem: string[]): string[] => { const f = dropStress(stem[stem.length - 1] ?? ""); return SIBILANT.has(f) ? ["IH0", "Z"] : VOICELESS.has(f) ? ["S"] : ["Z"]; };
  const allomorphED = (stem: string[]): string[] => { const f = dropStress(stem[stem.length - 1] ?? ""); return f === "T" || f === "D" ? ["IH0", "D"] : VOICELESS.has(f) ? ["T"] : ["D"]; };
  const SUFFIXES: [string, (w: string) => string[], (s: string[]) => string[]][] = [
    ...
    ["ing", (w) => [w.slice(0, -3), w.slice(0, -3) + "e", w.slice(0, -4)], () => ["IH0", "NG"]],
    ...
    ["er", (w) => [w.slice(0, -2), w.slice(0, -2) + "e", w.slice(0, -3), w.slice(0, -2).replace(/i$/, "y")], () => ["ER0"]],
    ["ly", (w) => [w.slice(0, -2), w.slice(0, -2).replace(/i$/, "y"), w.slice(0, -2) + "le"], () => ["L", "IY0"]],
    ["ed", (w) => [w.slice(0, -2), w.slice(0, -1), w.slice(0, -3)], (s) => allomorphED(s)],
  ...
  function morphDecode(w: string): string[] | null {
    for (const [suf, stems, allo] of SUFFIXES) {
      ...
      for (const stem of stems(w)) {
        ...
        const sp = dict.get(stem); if (!sp) continue;
        return [...sp, ...allo(sp)];
      }
    }
    return null;
  ...
  function compoundSplit(w: string): string[] | null {
    const n = w.length;
    const best: ({ parts: string[][]; nparts: number; minLen: number; score: number } | null)[] = new Array(n + 1).fill(null);
    ...
    for (let i = 0; i < n; i++) {
      ...
      for (let j = i + MINPART; j <= n; j++) {
        ...
        if (common.size > 0 && !common.has(piece) && !(j === n && j - i >= 5)) continue;
        let phones = dict.get(piece) ?? null;
        if (!phones && j === n && j - i >= 5) { const mp = morphDecode(piece); if (mp) phones = mp; }
        if (!phones) continue;
        const cand = { parts: [...best[i]!.parts, phones], nparts: best[i]!.nparts + 1, minLen: Math.min(best[i]!.minLen, j - i), score: best[i]!.score + (j - i) * (j - i) };
        const cur = best[j];
        if (!cur || cand.minLen > cur.minLen || (cand.minLen === cur.minLen && cand.score > cur.score)) best[j] = cand;
      }
    }
    const full = best[n];
  ...
  function decomposeInner(w: string): { phones: string[]; source: "C" | "M" | "N" } {
    const c = compoundSplit(w); if (c) return { phones: enforceSinglePrimary(collapseGeminates(c)), source: "C" };
    const m = morphDecode(w); if (m) return { phones: enforceSinglePrimary(collapseGeminates(m)), source: "M" };
    return { phones: enforceSinglePrimary(collapseGeminates(ngramDecode(w))), source: "N" };
  }
