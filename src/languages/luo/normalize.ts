...
export function normalizeLuo(input: string): string {
    ...
    s = rewrite(s, /(?<![\d.,])(\d+)\.(\d+)(?=\p{L}(?![\p{L}\p{M}]))/gu, "$1 $2");
    ...
    s = rewrite(s, new RegExp(`${NOT_BEFORE}(${MAGNITUDE}\\s+)?([$£])\\s?(?=\\d)`, "giu"),
    ...
    s = rewrite(s, /(?<![\d:.,])(\d{1,2}:\d{2})\s?-\s?(\d{1,2}:\d{2})(?![\d:.,])/gu, "$1 nyaka $2");
    ...
    s = rewrite(s, /(?<![\d.,:\-\/])(\d+(?:\.\d+)?)\s?-\s?(\d+(?:\.\d+)?)(?![\d\/\p{L}\p{M}])(?!\s?-\s?\d)/gu,
    ...
    s = rewrite(s, /(?<=(?:saa|Saa)\s)(?<![\d.,:])(\d{1,2})\.(\d{2})(?![\d.,:])/gu, "$1 $2");
    s = rewrite(s, /(?<![\d.,:])(\d{1,2})\.(\d{2})(?![\d.,:])(?=\s?(?:UTC|GMT))/gu, "$1 $2");
