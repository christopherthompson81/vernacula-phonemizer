...
export function normalizeTurkmen(input: string): string {
    ...
    s = s.replace(new RegExp(`(\\d)\\s?°\\s?C\\s?-\\s?(${SFX}{1,4})${NOT_LETTER}`, "gu"), "$1 gradus Selsi$2");
    s = s.replace(/(\d)\s?°\s?C(?![\p{L}\p{M}])/gu, "$1 gradus Selsi");
    s = s.replace(/(\d)\s?°\s?F(?![\p{L}\p{M}])/gu, "$1 gradus Farengeýt");
