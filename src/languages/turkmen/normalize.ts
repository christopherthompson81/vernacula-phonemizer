...
export function foldTurkmenTildes(input: string): string {
    ...
    return rewrite(input, /[\p{L}\p{M}]+/gu, (w) =>
...
export function normalizeTurkmen(input: string): string {
    ...
    s = rewrite(s, new RegExp(`${NOT_LETTER_BEFORE}mlrd\\.?${NOT_LETTER_AFTER}`, "giu"), "milliard");
    s = rewrite(s, new RegExp(`${NOT_LETTER_BEFORE}mln\\.?${NOT_LETTER_AFTER}`, "giu"), "million");
    ...
    for (const [re, word] of multi)
        s = rewrite(s, re, (m0: string, g1: string | undefined, offset: number, full: string) => {
            const out = `${word}${g1 ?? ""}`;
    ...
    s = rewrite(s, new RegExp(`(\\d+)\\s?%\\s?-\\s?(${SFX}{1,5})${NOT_LETTER_AFTER}`, "gu"), "$1 göterim$2");
    ...
    s = rewrite(s, new RegExp(`(\\d)\\s?ý\\.?${NOT_LETTER_AFTER}`, "gu"), "$1 ýyl");

    // 4) НОМЕР / NUMBER SIGN. The sign was dropped outright.
    s = rewrite(s, /№\s?(?=\d)/gu, "belgi ");
    ...
    s = rewrite(s, new RegExp(`(?<![\\d.,])(\\d+)\\s?-\\s?((?:[yiuü])?nj[yi]${SFX}{0,6})${NOT_LETTER_AFTER}`, "giu"),
    ...
    s = rewrite(s, /(?<![\d.,/])(\d{1,2})\s?\/\s?(\d{1,2})(?![\d.,/])/gu, (whole, num: string, den: string) => {
    ...
    s = rewrite(s, /±/gu, " plýus minus ");
    s = rewrite(s, /(^|[\s(])\+\s?(\d)/gu, "$1plýus $2");
    ...
    s = rewrite(s, /(\d)\s?°\s?(\d+)\s?[′']\s?(\d+)\s?[″"]/gu, "$1 gradus $2 minut $3 sekunt ");
    s = rewrite(s, /(\d)\s?°\s?(\d+)\s?[′']/gu, "$1 gradus $2 minut ");
    ...
    s = rewrite(s, new RegExp(`(\\d)\\s?°\\s?[Cc]\\s?-\\s?(${SFX}{1,4})${NOT_LETTER_AFTER}`, "gu"), "$1 gradus Selsi$2");
    s = rewrite(s, /(\d)\s?°\s?C(?![\p{L}\p{M}])/gui, "$1 gradus Selsi");
    s = rewrite(s, /(\d)\s?°\s?F(?![\p{L}\p{M}])/gui, "$1 gradus Farengeýt");
    //    ⚠ AND THE ABLATIVE SITS ON THE SIGN — `+10° dan demirgazyga` ("from +10° northward"). Glued, or
    //    the bare `dan` stands as its own word.
    s = rewrite(s, /(\d)\s?°\s?(dan|den)(?![\p{L}\p{M}])/gu, "$1 gradus$2");
