...
export function makeUrduNormalizer(numbers: NumbersDef): (text: string) => string {
    ...
    return (input: string): string => {
        ...
        s = rewrite(s, new RegExp(`(?<![\\d.,])(\\d+)\\s?(${Object.keys(SUFFIX_FORM).join("|")})(?![\\p{L}\\p{M}])`, "gu"),
        ...
        s = rewrite(s, /(\d)\s?°\s?C(?![\p{L}])/giu, "$1 ڈگری سینٹی گریڈ");
        s = rewrite(s, /(\d)\s?°\s?F(?![\p{L}])/giu, "$1 ڈگری فارن ہائیٹ");
        s = rewrite(s, /(\d)\s?°/gu, "$1 ڈگری");
        ...
        s = rewrite(s, /(?<![\d:])([01]?\d|2[0-3])\s?:\s?([0-5]\d)(?![\d:])(?!,\d)(\s*بجے)?/gu,
        ...
        s = rewrite(s, /(^|[\s(])[-−–](\d)/gu, "$1منفی $2");
        s = rewrite(s, /(\S)\+\s?(\d)/gu, "$1 جمع $2");
        s = rewrite(s, /(^|\s)\+\s?(\d)/gu, "$1جمع $2");
