...
export function normalizeTamil(input: string): string {
    ...
    let s = rewrite(input, /[​-‍﻿]/gu, "");  // ZWSP, ZWJ, BOM
    ...
    s = rewrite(s, /(?<=\d)(?<!(?<![\d])0),(?=\d{2,3}(?:,\d|[^\d]|$))/gu, "");
    ...
    s = rewrite(s, ERA_RE, (_m, k: string) => ERA[k]!);
    s = rewrite(s, new RegExp(`${NOT_LETTER_BEFORE}எ\\s*\\.\\s*கா\\s*\\.?${NOT_LETTER_AFTER}`, "gu"), "எடுத்துக்காட்டாக");
    ...
    s = rewrite(s, TAMIL_UNIT_DOT_RE, (_m, a: string, b: string) =>
        TAMIL_UNIT[`${a}${b}`] ?? TAMIL_UNIT[`${a}மீ`] ?? `${a}${b}`);
    s = rewrite(s, TAMIL_UNIT_SPACED_RE, (_m, a: string, b: string) =>
        TAMIL_UNIT[`${a}${b}`] ?? TAMIL_UNIT[`${a}மீ`] ?? `${a} ${b}`);
    s = rewrite(s, TAMIL_UNIT_RE, (_m, u: string) => TAMIL_UNIT[u]!);
    ...
    s = rewrite(s, new RegExp(`${NOT_LETTER_BEFORE}திரு\\s*\\.\\s*(?=[\\p{L}])`, "gu"), "திரு ");
    ...
    s = rewrite(s, new RegExp(`(\\d[\\d.]*)\\s?mph${NOT_LETTER_AFTER}`, "giu"),
    ...
    s = rewrite(s, /±/gu, " கூட்டல் கழித்தல் ");
    s = rewrite(s, /(\S)\+\s?(\d)/gu, "$1 பிளஸ் $2");
    s = rewrite(s, /(^|\s)\+\s?(\d)/gu, "$1பிளஸ் $2");

    // 8c) THE RELATIONAL AND DIVISION SIGNS
    s = postposedSign(s, "<", "ஐ விட குறைவாக");
    s = postposedSign(s, ">", "ஐ விட அதிகமாக");
    s = rewrite(s, /\s?=\s?/gu, " சமம் ");
    s = rewrite(s, /\s?÷\s?/gu, " வகுத்தல் ");
    ...
    s = rewrite(s, ORDINAL_RE, (whole, digits: string, suffix: string) =>
    ...
    s = rewrite(s, /(\d)\s*-?\s*ல்(?![\p{L}\p{M}])/gu, "$1 இல்");
