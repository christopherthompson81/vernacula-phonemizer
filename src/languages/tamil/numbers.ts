...
const ONES = N.units, TENS = N.tens, M = N.magnitudes;
...
function below1000(n: number): string[] {
  ...
  const parts: string[] = [];
  if (h === 1) parts.push(M.hundred);
  else if (h > 1) parts.push(ONES[h]!, M.hundred);
...
export function numberToWords(n: number): string {
  ...
  if (crore > 0) parts.push(...below1000(crore), M.crore);
  if (lakh > 0) parts.push(...below100(lakh), M.lakh);
  if (thou > 0) parts.push(...below1000(thou), M.thousand);
