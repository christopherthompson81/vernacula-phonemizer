...
export function normalizeMalayalam(input: string): string {
    ...
    let s = rewrite(input, /([ണനരലളക])്\u200d/gu, (_m, base: string) => ZWJ_CHILLU[base]!);
    ...
    s = rewrite(s, clitic(ORDINAL_ENDINGS), (whole, digits: string, end: string) => {
    ...
    s = rewrite(s, clitic(PLURAL_CLITICS), (whole, digits: string, c: string) => {
        const n = Number(digits);
        if (!Number.isSafeInteger(n) || n === 0) return whole;
    ...
    s = rewrite(s, clitic(OBLIQUE_CLITICS), (whole, digits: string, c: string) => {
        const n = Number(digits);
        if (!Number.isSafeInteger(n) || n === 0) return whole;
    ...
    s = rewrite(s, /ശതമാനം(\s+ശതമാനം)+/gu, "ശതമാനം");
