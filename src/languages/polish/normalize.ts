...
export function normalizePolish(input: string): string {
    ...
    s = rewrite(s, /(\d+)\s?Mb\s?\/\s?s(?![\p{L}\p{M}])/gu, "$1 megabitów na sekundę");
    ...
    s = rewrite(s, new RegExp(`(?<![\\p{L}\\p{M}])(?:(${PREP_ALT})\\s+)?godz\\.`, "giu"),
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}\d.,])(lat|lata|latach)(\s+)(\d+)\.(?=\s*[,\p{Ll}])/gu,
    ...
    s = rewrite(s, /([01]?\d|2[0-3]):([0-5]\d)(?![\d:])(?!,\d)/gu,
    ...
    s = rewrite(s, /\s?=\s?/gu, " równa się ");
    s = rewrite(s, /\s?<\s?/gu, " mniejsze niż ");
    s = rewrite(s, /\s?>\s?/gu, " większe niż ");
    s = rewrite(s, /\s?÷\s?/gu, " podzielić przez ");
