...
/**
 * ⚠ THE THREE OVER-THE-VOWEL MARKS (՛ ՜ ՞) ARE NO LONGER HANDLED HERE. They were this file's step 0, and
 * the rule was right — but the word they break is broken by the SHARED tokenizer, so Western Armenian
 * inherited the defect and not the fix. The rule now lives in `armenian.ts` as `unbreakMarks`, applied to
 * the input BEFORE this normalizer runs, which preserves the ordering step 0 needed and gives hyw the same
 * reading. The measurement that motivated it (FLEURS `hy_am` + `tools/corpus/mined/hy.jsonc`, 4,465 unique
 * lines: ՛ on 54 lines, 32 of them inside a word; ՞ inside a word ×15, every one an interrogative; none of
 * it in the parity golden) is quoted in that docstring.
 */
