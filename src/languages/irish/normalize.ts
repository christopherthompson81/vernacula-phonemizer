...
export function normalizeIrish(input: string): string {
    ...
    s = rewrite(s, /[\u200B\u200C\u200D\uFEFF]/gu, "");  // ZWSP, ZWNJ, ZWJ, BOM
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])srl\.?(?![\p{L}\p{M}])/giu, "agus araile");
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])(?:US|uS)\$(?=\s?\d)/giu, "dollar na Stát Aontaithe ");
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])Dr\.(\s+)(?=[\p{L}\d])/giu, "Dochtúir$1");
    s = rewrite(s, /(?<![\p{L}\p{M}])Dr\.(?=\s*(?:[.,;:!?»)]|$))/giu, "Dochtúir.");
    s = rewrite(s, /(?<![\p{L}\p{M}])etc\.(\s+)(?=[\p{L}\d])/giu, "srl$1");
    s = rewrite(s, /(?<![\p{L}\p{M}])etc\.(?=\s*(?:[.,;:!?»)]|$))/giu, "srl.");
    ...
    s = rewrite(s, /(?<![\d.,])(\d(?:[\d,]*\d)?)\s*[-–]\s*(\d(?:[\d,]*\d)?)(?![\d.])/gu, "$1 go dtí $2");
    ...
    s = rewrite(s, /(?<![\d:,])([01]?\d|2[0-3]):([0-5]\d)(?![:.\d])(?:\s*(i\.?n\.?|r\.?n\.?|[Aa]\.?[Mm]\.?|[Pp]\.?[Mm]\.?))?/giu,
    ...
    s = rewrite(s, /(\d+)¾/gu, "$1 agus trí cheathrú");
    s = rewrite(s, /(\d+)½/gu, "$1 agus leath");
    ...
    s = rewrite(s, /(\d)\s?[°º]\s?C(?![\p{L}\p{M}])/giu, "$1 céim Celsius");
    s = rewrite(s, /(\d)\s?[°º]\s?F(?![\p{L}\p{M}])/giu, "$1 céim Fahrenheit");
    s = rewrite(s, /(\d)\s?[°º]\s?([NSEW])(?![\p{L}\p{M}])/giu,
        (_m, d: string, c: string) =>
            `${d} céim ${({ N: "ó thuaidh", S: "ó dheas", E: "soir", W: "siar" } as Record<string, string>)[c.toUpperCase()]!}`);
    s = rewrite(s, /(\d)\s?[°º](?![\p{L}\p{M}])/gu, "$1 céim");
    ...
    s = rewrite(s, /(?<!\d)(\d+)\s?(km|m|kg|mm|cm)\s*\/\s*(h|u)(?![\p{L}\p{M}])/giu,
    ...
    s = rewrite(s, /(?<!\d)(\d+)\s?msu(?![\p{L}\p{M}])/giu, (m0, n: string) =>
    ...
    s = rewrite(s, /±/gu, " móide nó lúide ");
    s = rewrite(s, /\+\s?(?=\d)/gu, " móide ");
    ...
    s = rewrite(s, /(?<![\p{L}\p{Nd}])[-−](\d+)(?!\s*[-\d])/gu, "lúide $1");
    ...
    s = rewrite(s, /(\S)\s*÷\s*(\S)/gu, "$1 roinnt ar $2");
    s = rewrite(s, /(\d)\s*<\s*(\d)/gu, "$1 níos lú ná $2");
    s = rewrite(s, /(\d)\s*>\s*(\d)/gu, "$1 níos mó ná $2");
    s = rewrite(s, /(\d)\s*×\s*(\d)/gu, "$1 faoi $2");
    ...
    s = rewrite(s, /([\p{L}\p{M}\d]+ pointe [\p{L}\p{M} ]+?)\s*%\s*(?![\p{L}\p{M}])/gu, "$1 faoin gcéad");
