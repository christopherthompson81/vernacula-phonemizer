...
export function normalizeJapanese(input: string): string {
    ...
    s = rewrite(s, DECLARED_RUBY, (_m, b1?: string, r1?: string, _b2?: string, r2?: string) =>
        (r1 ?? r2 ?? b1) as string);
    ...
    s = rewrite(s, PARENTHESISED_RUBY, (whole, base: string, r1?: string, r2?: string) => {
        const ruby = r1 ?? r2 ?? "";
    ...
    s = rewrite(s, /(?<![\d/])(\d{1,3})\/(\d{1,3})(?![\d/])/gu, "$2ブンノ$1");
    ...
    s = rewrite(s, /(?<=[\d\p{Script=Han}\p{sc=Katakana}])[〜～~](?=\d)/gu, "から");
    ...
    s = rewrite(s, /(\d)\s?(?:℃|°\s?C)(?![\p{sc=Latn}])/gui, "$1度");
    s = rewrite(s, /(\d)\s?(?:℉|°\s?F)(?![\p{sc=Latn}])/gui, "華氏$1度");
    s = rewrite(s, /(\d)\s?°/gu, "$1度");

    // 9) SIGNS. Neither occurs in this corpus, but a dropped sign is silent content loss wherever it does.
    s = rewrite(s, /(^|[\s(（])[-−–](\d)/gu, "$1マイナス$2");
    ...
    s = rewrite(s, /±/gu, " プラスマイナス ");
    s = rewrite(s, /(^|[\s(（])\+\s?(\d)/gu, "$1プラス$2");
    s = rewrite(s, /(\S)\+\s?(\d)/gu, "$1プラス$2");
    ...
    s = rewrite(s, /(\d)\s?<\s?(\d)/gu, "$1は$2より小さい");
    s = rewrite(s, /(\d)\s?>\s?(\d)/gu, "$1は$2より大きい");
    s = rewrite(s, /\s?=\s?/gu, "イコール");
    s = rewrite(s, /\s?÷\s?/gu, "わる");
