...
export function normalizeKirundi(input: string): string {
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}\d.,:/-])(\d+)[ \u00a0]?[-–—/][ \u00a0]?(\d+)[ \u00a0]?°[ \u00a0]?[CF]?(?![\p{L}\p{M}])/gui,  // space, NBSP
        (w, a: string, b: string, off: number, full: string) =>
            Number(a) < Number(b)
                ...
                : w);
    s = rewrite(s, new RegExp(`(?<=${DEGREE}[ \u00a0])(\\d+)[ \u00a0]?[-–—/][ \u00a0]?(\\d+)(?![\\d.,:/])`, "giu"),  // space, NBSP
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}\d])([-−–]?)(\d+(?:[.,]\d+)?)[ \u00a0]?°(?=[ \u00a0]?\d+[′'])/gu,  // space, NBSP
        (w, sg: string, n: string, off: number, full: string) => `${degreeBody(sg, n, off, off + w.length, full)} `);
    //    6c) A BARE DEGREE.
    s = rewrite(s, /(?<![\p{L}\p{M}\d])([-−–]?)(\d+(?:[.,]\d+)?)[ \u00a0]?[°º](?![\p{L}\p{M}])/gu,  // space, NBSP
        (w, sg: string, n: string, off: number, full: string) => degreeBody(sg, n, off, off + w.length, full));
    ...
    s = rewrite(s, /(\d)[ \u00a0]?%/gu, (w, d: string, off: number, full: string) =>  // space, NBSP
