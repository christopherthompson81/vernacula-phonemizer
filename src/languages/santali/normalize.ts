...
export function normalizeSantali(input: string): string {
    ...
    s = s.replace(new RegExp(`(?<=${D})${GAAHLAA}(?=${D})`, "gu"), ".");
    ...
    s = s.replace(new RegExp(`(?<=${O})-(ᱟᱜ?|ᱟᱭ|ᱮ)(?![${OL}])`, "gu"), `${PHAARKAA}$1`);
    ...
    s = s.replace(new RegExp(`(?<=${A})\\.(?=${O})`, "gu"), GAAHLAA);
    s = s.replace(new RegExp(`(?<=${O}${O}${A})\\.(?![${OL}.])`, "gu"), GAAHLAA);
    s = s.replace(new RegExp(`(?<=${O})\\.(?=\\s*${O})`, "gu"), " ");
    s = s.replace(new RegExp(`(?<=${O})\\.(?!\\s*[${OL}])(?=\\s|$|[),])`, "gu"), " ");
    ...
    s = s.replace(new RegExp(`(${D})\\s*°(?![\\p{sc=Latn}${D}′″'"])`, "gu"), "$1 ᱰᱤᱜᱨᱤ");
