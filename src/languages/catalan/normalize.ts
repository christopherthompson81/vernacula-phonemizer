...
export function normalizeCatalan(input: string): string {
    ...
    for (const [body, word] of MULTI_DOT) {
        s = rewrite(s, new RegExp(`(?<![\\p{L}\\p{M}])${body}(?![\\p{L}\\p{M}])`, "giu"), word);
    }
    ...
    s = rewrite(s, new RegExp(`(?<![\\p{L}\\p{M}])(dr|etc)\\.(\\s+)(?=[\\p{L}\\d])`, "giu"),
    ...
    s = rewrite(s, new RegExp(`(?<![\\p{L}\\p{M}])(dr|etc)\\.(?=\\s*(?:[.,;:!?»)]|$))`, "giu"),
        (m0, ab: string) => {
            // ⚠ THE MISS BRANCH IS REACHABLE (#1122): the pattern is built from this table's own
            // keys but carries `i`+`u`, so JS's fold widens it and a near-miss matches while its
            // key is absent. The `!` here made `String.replace` stringify `undefined`.
    ...
    s = rewrite(s, /(?<![\d.,])(\d+)(è|a|r|n|ns|t)(?![\p{L}\p{M}])/giu, (m0, d: string, sfx: string) => {
        const n = Number(d);
        ...
        if (m === "ns") return rewrite(ord, /segon$/u, "segons");
        return ord;
    });
    ...
    s = rewrite(s, /(?<![\d.,])((?:1\d|20)\d{2}|\d0)s(?![\p{L}\p{M}])/giu, "$1");
    ...
    s = rewrite(s, /(?<![\d:,])([01]?\d|2[0-3]):([0-5]\d)(?![:.\d])\s*(h)?\s*([Aa]\.?[Mm]\.?|[Pp]\.?[Mm]\.?)?/giu,
    ...
    s = rewrite(s, /(?<![\d.,])(\d+)\.(\d{1,2})(?![\d.])/giu, "$1 punt $2");
    ...
    s = rewrite(s, /(\d+)¾/gu, "$1 i tres quarts");
    s = rewrite(s, /(\d+)½/gu, "$1 i mig");
    ...
    s = rewrite(s, /(\d)\s?[°º]\s?C(?![\p{L}\p{M}])/giu, "$1 graus Celsius");
    s = rewrite(s, /(\d)\s?[°º]\s?F(?![\p{L}\p{M}])/giu, "$1 graus Fahrenheit");
    ...
    s = rewrite(s, /(\d)\s?[°º]\s?([NSOE])(?![\p{L}\p{M}])/giu, (_m, d: string, c: string) =>
        `${d} graus ${({ N: "nord", S: "sud", E: "est", O: "oest" } as Record<string, string>)[c.toUpperCase()]!}`);
    s = rewrite(s, /(\d)\s?[°º](?![\p{L}\p{M}])/gu, "$1 graus");
    ...
    s = rewrite(s, /(\d+(?: punt \d+)?)\s?Ghz?(?![\p{L}\p{M}])/giu, "$1 gigahercis");
    ...
    s = rewrite(s, /±/gu, " més menys ");
    s = rewrite(s, /\+\s?(?=\d)/gu, " més ");
    ...
    s = rewrite(s, /(?<![\p{L}\p{Nd}])[-−](\d+)(?!\s*[-\d])/gu, "menys $1");
    ...
    s = rewrite(s, /(\S)\s*=\s*(\S)/gu, "$1 és igual a $2");
    ...
    s = rewrite(s, /(\S)\s*÷\s*(\S)/gu, "$1 dividit per $2");
    s = rewrite(s, /(\d)\s*<\s*(\d)/gu, "$1 és menor que $2");
    s = rewrite(s, /(\d)\s*>\s*(\d)/gu, "$1 és major que $2");
    s = rewrite(s, /(\d)\s*×\s*(\d)/gu, "$1 per $2");
