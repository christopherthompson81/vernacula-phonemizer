...
export function normalizeHebrew(input: string): string {
    ...
    s = rewrite(s, /[‎‏‪-‮⁦-⁩​-‍﻿]/gu, "");  // LRM, RLM, LRE, RLO, LRI, PDI, ZWSP, ZWJ, BOM
    s = rewrite(s, /&nbsp;|&#(?:x[0-9a-f]+|\d+);/giu, " ");
    ...
    s = rewrite(s, /(?<![0-9.,])[1-9][0-9]{0,2}(?:,[0-9]{3})+(?![0-9]|,[0-9])/gu, (w) => w.replace(/,/gu, ""));
    ...
    s = rewrite(s, new RegExp(`\\$\\s?(${NUM})`, "gu"), "$1 דּוֹלָר ");
    s = rewrite(s, new RegExp(`(${NUM}${MAG})\\s?\\$`, "gu"), "$1 דּוֹלָר ");
    ...
    s = rewrite(s, new RegExp(`(?<![0-9.,])(${NUM})\\s?°\\s?[CF]${NOT_LETTER_AFTER}`, "gui"), "$1 מַעֲלוֹת צֶלְזִיוּס ");
    s = rewrite(s, new RegExp(`(?<![0-9.,])(${NUM})\\s?°`, "gu"), "$1 מַעֲלוֹת ");
    ...
    s = rewrite(s, new RegExp(`(?<![\\p{L}\\p{M}0-9.])(${NUM})\\s?km³${NOT_LETTER_AFTER}`, "gu"), "$1 קִילוֹמֶטֶר מְעֻקָּב ");
    s = rewrite(s, new RegExp(`(?<![\\p{L}\\p{M}0-9.])(${NUM})\\s?km²${NOT_LETTER_AFTER}`, "gu"), "$1 קִילוֹמֶטֶר רָבוּעַ ");
    s = rewrite(s, new RegExp(`(?<![0-9.,])(${NUM})\\s?²`, "gu"), "$1 בְּרִיבּוּעַ ");
