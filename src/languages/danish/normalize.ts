...
export function normalizeDanish(input: string): string {
    ...
    t = rewrite(t, /(\d)\s*°(?=[\p{L}\p{M}])/gu, "$1 grader ");
    ...
    t = rewrite(t, /(?<!\d)(\d{1,2})\.\s+(og|eller)\s+(\d{1,2})\./gu, (m, a: string, conj: string, b: string) => {
    ...
    t = rewrite(t, /\b([A-Z]{2,3})\s*\$/gu, "$1 dollar");
    for (const [sign, word] of Object.entries(CURRENCY)) {
        ...
        t = rewrite(t, new RegExp(`${esc}\\s*(\\d+)(?=[\\p{L}\\p{M}])`, "gu"), `$1 ${word} `);
