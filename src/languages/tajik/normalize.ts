 ...
 * садум ×14, сиюм ×20.
 */
function ordinalEnding(word: string): string {
    if (word === "сӣ") return "сиюм";
    return /[аоеиуӯёюяӣэ]$/u.test(word) ? `${word}юм` : `${word}ум`;
}

/** Integer → the Tajik ordinal in ORTHOGRAPHY (emitted as text, phonemized by the g2p). */
export function tajikOrdinal(n: number): string | undefined {
    if (!Number.isInteger(n) || n < 1 || n > 999_999) return undefined;
    const parts = numberWords(n).split(" ");
    const last = parts[parts.length - 1];
    if (last === undefined || last === "") return undefined;
    parts[parts.length - 1] = ordinalEnding(last);
    return parts.join(" ");
}
 ...
 */
function glueSuffix(words: string, suffix: string): string {
...
const SUFFIX = "ро|и|ест|аш|ат|ам|он|ҳо";
 ...
 */
export function normalizeTajikInitialisms(text: string): string {
...
export function normalizeTajik(input: string): string {
    let s = input;
    ...
    for (let i = 0; i < 3; i++) s = rewrite(s, /(?<=\d)(?<!(?<![\d\.,])0)[ \u00a0\u202f\u2009](?=\d{3}(?![\d]))/gu, "");  // space, NBSP, NNBSP, thin space
    ...
    s = rewrite(rewrite(rewrite(rewrite(rewrite(rewrite(s, /(?<=\d\s?)млрд\.(?=\s+[\p{Ll}\d])/gu, "миллиард")
        , /(?<=\d\s?)млн\.(?=\s+[\p{Ll}\d])/gu, "миллион")
        , /(?<=\d\s?)ҳаз\.(?=\s+[\p{Ll}\d])/gu, "ҳазор")
        , /(?<=\d\s?)млрд(?![\p{L}\p{M}])/gu, "миллиард")
        , /(?<=\d\s?)млн(?![\p{L}\p{M}])/gu, "миллион")
        , /(?<=\d\s?)ҳаз(?![\p{L}\p{M}])/gu, "ҳазор");
    ...
    s = rewrite(s, /(?<![\d.,])(\d+)-(ум|юм)([\p{L}\p{M}]*)/gu, (m0, d: string, _e: string, tail: string) => {
    ...
    s = rewrite(s, new RegExp(`(\\d[\\d,]*)\\s?%-(${SUFFIX})(?![\\p{L}\\p{M}])`, "gu"),
    ...
    s = rewrite(s, new RegExp(`(?<![\\d.,])(\\d+)-(${SUFFIX})(?![\\p{L}\\p{M}])`, "gu"),
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])(\d+)\s?[-–—]\s?(\d)/gu, "$1 то $2");
    ...
    s = rewrite(s, new RegExp(`(\\d)\\s?°\\s?[CСcс](?:-(${SUFFIX}))?(?![\\p{L}\\p{M}])`, "gu"),
        (_m, d: string, sfx: string) => `${d} дараҷаи Селсий${sfx ?? ""}`);
    s = rewrite(s, /(\d)\s?°\s?F(?![\p{L}\p{M}])/gui, "$1 дараҷаи Фаренгейт");
    ...
    s = rewrite(s, /(\d)\s?°\s?([NSEW])(?![\p{L}\p{M}])/gu, (_m, d: string, c: string) =>
    ...
    s = rewrite(s, new RegExp(`(\\d)\\s?°(?:-(${SUFFIX}))?`, "gu"),
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])([Ѐ-ӿ]{2,})\s?\/\s?км(²|2)(?![\p{L}\p{M}\d])/gu, "$1 дар километри мураббаъ");
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}\d./])(\d{1,2})\s?\/\s?(\d{1,2})(?![\d./])/gu, (m0, a: string, b: string) => {
        ...
        return den === undefined ? m0 : `${numberWords(n)} ${den}`;
    });
    ...
    s = rewrite(s, /([\dЀ-ӿ])\s*=\s*([\dЀ-ӿ])/gu, "$1 баробар аст ба $2");
