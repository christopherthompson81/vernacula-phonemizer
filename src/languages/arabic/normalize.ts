...
export function foldLetterforms(input: string): string {
    return rewrite(rewrite(input, /ی/gu, "ي"), /ک/gu, "ك");
}
...
export function normalizeArabic(input: string): string {
    ...
    s = rewrite(s, new RegExp(`([${DIGIT}])\\s?(${UNIT_ALT})(?![\\p{L}\\p{M}])`, "gu"),
    ...
    s = rewrite(s, new RegExp(`([${DIGIT}])\\s?°\\s?C(?![\\p{L}])`, "giu"), "$1 دَرَجَة مِئَوِيَّة");
    s = rewrite(s, new RegExp(`([${DIGIT}])\\s?°\\s?F(?![\\p{L}])`, "giu"), "$1 دَرَجَة فَهْرَنْهَايْت");
    s = rewrite(s, new RegExp(`([${DIGIT}])\\s?°`, "gu"), "$1 دَرَجَة");
    ...
    s = rewrite(s, new RegExp(`(الساعة\\s*)?(?<![${DIGIT}:])([${DIGIT}]{1,2}):([${DIGIT}]{2})(?![${DIGIT}:])`, "gu"),
    ...
    //    an instance.
    s = rewrite(s, new RegExp(`(^|[\\s(])[-−–]([${DIGIT}])`, "gu"), "$1نَاقِص $2");
    ...
    s = rewrite(s, /±/gu, " زَائِد نَاقِص ");
    s = rewrite(s, new RegExp(`(\\S)\\+\\s?([${DIGIT}])`, "gu"), "$1 زَائِد $2");
    s = rewrite(s, new RegExp(`(^|\\s)\\+\\s?([${DIGIT}])`, "gu"), "$1زَائِد $2");
    ...
    s = rewrite(s, /\s?=\s?/gu, " يُسَاوِي ");
    s = rewrite(s, /\s?<\s?/gu, " أَصْغَر مِن ");
    s = rewrite(s, /\s?>\s?/gu, " أَكْبَر مِن ");
    s = rewrite(s, /\s?÷\s?/gu, " مَقْسُوم عَلَى ");
    ...
    s = rewrite(s, new RegExp(`\\s*(?:×|(?<=[${DIGIT}])x)\\s*(?=[${DIGIT}])`, "gu"), " في ");
    ...
    s = rewrite(s, new RegExp(`(?<![${DIGIT}.,/])([${DIGIT}]{1,3})/([${DIGIT}]{1,3})(?![${DIGIT}/])`, "gu"),
        (_m, a: string, b: string) => `${a} عَلَى ${b}`);

    return rewrite(s, ARABIC_DIGIT, foldDigit);
}
