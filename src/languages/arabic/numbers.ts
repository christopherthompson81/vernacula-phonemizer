...
const {
    ...
    teens: TEENS,
    tens: TENS,
    hundredsConstruct: HUNDREDS_CONSTRUCT,
    connector: WA,
    magnitudes: MAG,
} = MANIFEST.numbers;
...
function below1e6(n: number): string {
    ...
    if (th === 1) head = MAG.thousand;
        ...
        head = `${below100(th)} ${MAG.thousandsPlural}`; // 3–10 thousand: plural ʔaːlaːf
...
export function numberToIpa(n: number): string {
    ...
    if (m === 1) head = MAG.million;
        ...
        head = MAG.millionDual; // dual
    else if (m <= 10)
        head = `${below100(m)} ${MAG.millionsPlural}`; // 3–10 million: plural malaːjiːn
