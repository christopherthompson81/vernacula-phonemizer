...
export function normalizeWelsh(input: string): string {
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])O\.C\.(?![\p{L}\p{M}])/giu, "Oed Crist");
    s = rewrite(s, /(?<![\p{L}\p{M}])C\.C\.(?![\p{L}\p{M}])/giu, "Cyn Crist");
    s = rewrite(s, /(?<![\p{L}\p{M}])OC(?=\s*(?:\d+)\s*[–-]\s*\d+)/giu, "Oed Crist");
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])AUD\$?(?=\s?\d)/giu, "doler Awstralia ");
    s = rewrite(s, /(?<![\p{L}\p{M}])US\$?(?=\s?\d)/giu, "doler yr Unol Daleithiau ");
    s = rewrite(s, /(?<![\p{L}\p{M}])U\.D\.(?![\p{L}\p{M}])/giu, "Unol Daleithiau");
    s = rewrite(s, /(?<![\p{L}\p{M}])UD(?=[\s,.]|$)(?![\p{L}\p{M}])/giu, "Unol Daleithiau");
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])DU(?=[\s,.]|$)(?![\p{L}\p{M}])/gu, "Deyrnas Unedig");
    s = rewrite(s, /(?<![\p{L}\p{M}])UDA(?=[\s,.]|$)(?![\p{L}\p{M}])/gu, "Unol Daleithiau America");
    s = rewrite(s, /(?<![\p{L}\p{M}])AS(?=[\s,.]|$)(?![\p{L}\p{M}])/gu, "Aelod Seneddol");
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])ayb\.(\s+)(?=[\p{L}\d])/giu, "ac yn y blaen$1");
    s = rewrite(s, /(?<![\p{L}\p{M}])ayb\.(?=\s*(?:[.,;:!?»)]|$))/giu, "ac yn y blaen.");
    ...
    //    as a time.
    s = rewrite(s, /(?<![\d.,])(\d[\d,]*)(fed|ed|af|eg|ydd|ain)(?![\p{L}\p{M}])/giu, (m0, d: string, sfx: string) => {
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}\d])(\d[\d,]*)(au)(?![\p{L}\p{M}])/giu, "$1");
    ...
    //     strips it and fuses them: `GMT-00:43` read *tˈidim*, one word. The initialism pass runs
    ...
    s = rewrite(s, /(?<![\d:,])([01]?\d|2[0-3]):([0-5]\d)\s*[-–]\s*([01]?\d|2[0-3]):([0-5]\d)(?![:.\d])/gu,
    ...
    s = rewrite(s, /(?<![\d.,])(\d(?:[\d,]*\d)?)\s*[-–]\s*(\d(?:[\d,]*\d)?)(?![\d]|\.\d)(\s?(?:km|kg|mm|cm|m)(?![\p{L}\p{M}]))?(?![%\p{Sc}])/gu,
    ...
    s = rewrite(s, /(?<![\d:,])([01]?\d|2[0-3]):([0-5]\d)(?![:.\d])(?:\s*(p\.?m\.?|a\.?m\.?)(?![\p{L}\p{M}]))?/giu,
    ...
    s = rewrite(s, /(?<![\d.,])([01]?\d|2[0-3])\.([0-5]\d)\s*(UTC|GMT)/giu,
    ...
    s = rewrite(s, /(?<![\d.,])(\d+\.\d+)\s*[-–]\s*(\d+\.\d+)(?![\d.])/gu, "$1 i $2");
    ...
    s = rewrite(s, /(?<![\d.,])(\d+)\.(\d+)\s?(km|m|kg|mm|cm)(?![\p{L}\p{M}])/giu,
    ...
    s = rewrite(s, /(\d+)¾/gu, "$1 a thri chwarter");
    s = rewrite(s, /(\d+)½/gu, "$1 a hanner");
    ...
    //    degree word.
    s = rewrite(s, /(\d)\s?[°º]\s?C(?![\p{L}\p{M}])/giu, "$1 gradd Celsius");
    s = rewrite(s, /(\d)\s?[°º]\s?F(?![\p{L}\p{M}])/giu, "$1 gradd Ffahrenheit");
    s = rewrite(s, /(\d)\s?[°º](?![\p{L}\p{M}])/gu, "$1 gradd");
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])cilomedr\/awr(?![\p{L}\p{M}])/giu, "cilomedr yr awr");
    s = rewrite(s, /(?<![\p{L}\p{M}])llath\/metr(?![\p{L}\p{M}])/giu, "llath neu fetr");
    ...
    s = rewrite(s, /±/gu, " plws minws ");
    s = rewrite(s, /\+\s?(?=\d)/gu, " plws ");
    ...
    s = rewrite(s, /(?<![\p{L}\p{Nd}])[-−](\d+)(?!\s*[-\d])/gu, "minws $1");
    ...
    s = rewrite(s, /(\S)\s*=\s*(\S)/gu, "$1 yn hafal i $2");
    ...
    s = rewrite(s, /(\S)\s*÷\s*(\S)/gu, "$1 rhannu â $2");
    s = rewrite(s, /(\d)\s*<\s*(\d)/gu, "$1 yn llai na $2");
    s = rewrite(s, /(\d)\s*>\s*(\d)/gu, "$1 yn fwy na $2");
    s = rewrite(s, /(\d)\s*×\s*(\d)/gu, "$1 gwaith $2");
