...
export function normalizeGeorgian(input: string): string {
    ...
    for (let i = 0; i < 2; i++) s = rewrite(s, /(?<=\d)(?<!(?<![\d])0)[ \u00a0\u202f\u2009](\d{3})(?!\d)/gu, "$1");  // space, NBSP, NNBSP, thin space
    ...
    s = rewrite(s, /(?<![\d.,])([1-9]\d{0,2})((?:,\d{3})+)(?![\d.,])/gu, (_m, head: string, rest: string) =>
    ...
    s = rewrite(s, /−/gu, " მინუს ");
    s = rewrite(s, /\((-)(?=\s?\d)/gu, "( მინუს ");
    s = rewrite(s, /(^|[\s(])[-–](?=\s?\d[\d.,]*\s?°)/gu, "$1მინუს ");
    ...
    s = rewrite(s, /(^|[\s(])\+\s?(?=\d)/gu, "$1პლუს ");
    s = rewrite(s, /(?<=\d)\s?\+\s?(?=\d)/gu, " პლუს ");
    ...
    s = rewrite(s, /(?<=[\d\p{Script=Georgian}])\s*=\s*(?=(?:მინუს\s+)?\d)/gu, " უდრის ");
    //     `№`, preposed. One instance (`ქუჩა № 37`), and the sign was dropped outright.
    s = rewrite(s, /№\s?(?=\d)/gu, "ნომერი ");
    ...
    s = rewrite(s, /(?<![\d:.,])(\d{1,2}):([0-5]\d)(?![\d:])(?=\s*(?:UTC|GMT))/gu,
    ...
    //        from inside a word.
    s = rewrite(s, new RegExp(`${NOT_LETTER_BEFORE}მე-(\\d{1,4})(?![\\d.,])`, "gu"), (m0, d: string) =>
        ordinalWord(Number(d)) ?? m0);
    //    4b) `N-ე` — the suffix half, ×6 (`179-ე`, `25-ე`, `41-ე`).
    s = rewrite(s, new RegExp(`(?<![\\d.,])(\\d{1,4})-ე${NOT_LETTER_AFTER}`, "gu"), (m0, d: string) =>
        ordinalWord(Number(d)) ?? m0);
    ...
    s = rewrite(s, new RegExp(`(?<![\\d.,])1-ლი${NOT_LETTER_AFTER}`, "gu"), "პირველი");
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])([IVX])(?![\p{L}\p{M}])(\s+)(?=საუკუნ|ათასწლეულ)/gu,
    ...
    s = rewrite(s, /(?<![\d.,\p{L}\p{M}])(\d{1,2})(?=\s+(?:საუკუნ|ათასწლეულ))/gu, (m0, d: string) => {
        const n = Number(d);
    ...
    s = rewrite(s, new RegExp(`(\\d[\\d.,]*)\\s?°\\s?C${DEG}`, "gui"), (_m, d: string, sfx: string | undefined) =>
        attach(`${figureToWords(d)} გრადუსი ცელსიუსი`, sfx));
    s = rewrite(s, new RegExp(`(\\d[\\d.,]*)\\s?°\\s?F${DEG}`, "gui"), (_m, d: string, sfx: string | undefined) =>
    ...
    s = rewrite(s, new RegExp(`(\\d[\\d.,]*)\\s?°${DEG}`, "gu"), (_m, d: string, sfx: string | undefined) =>
    ...
    s = rewrite(s, new RegExp(`%${DEG}`, "gu"), (_m, sfx: string | undefined) => attach("პროცენტი", sfx));
    ...
    s = rewrite(s, new RegExp(`(\\d[\\d.,]*)\\s?კვ\\.?\\s?(${UNIT_ALT})${DEG}`, "gu"),
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])კვტ\s?\/\s?სთ(?![\p{L}\p{M}])/gu, "კილოვატ საათი");
    s = rewrite(s, new RegExp(`${NOT_LETTER_BEFORE}(${UNIT_ALT})\\s?\\/\\s?(${UNIT_ALT})${NOT_LETTER_AFTER}`, "gu"),
        (_m, a: string, b: string) => `${UNIT_WORD.get(a)} ${attach(UNIT_WORD.get(b)!, b === "სთ" ? "ში" : "ზე")}`);
    s = rewrite(s, new RegExp(`(${GEO}+)\\s?\\/\\s?(${UNIT_ALT})([²³])?${NOT_LETTER_AFTER}`, "gu"),
    ...
    s = rewrite(s, new RegExp(`(\\d[\\d.,]*)\\s?${NOT_LETTER_BEFORE}(${UNIT_ALT})([²³])${DEG}`, "gu"),
        (_m, d: string, u: string, ex: string, sfx: string | undefined) =>
            attach(`${figureToWords(d)} ${exponentNoun(u, ex)}`, sfx));
    s = rewrite(s, new RegExp(`${NOT_LETTER_BEFORE}(${UNIT_ALT})([²³])${DEG}`, "gu"),
    ...
    s = rewrite(s, new RegExp(`(\\d[\\d.,]*)\\s?${NOT_LETTER_BEFORE}(${UNIT_ALT})${DEG}${NOT_LETTER_AFTER}`, "gu"),
    ...
    for (const [abbr, word] of SCALES)
        s = rewrite(s, new RegExp(`(?<=\\d)\\s?${NOT_LETTER_BEFORE}${abbr}\\.?${DEG}${NOT_LETTER_AFTER}`, "gu"),
    ...
    for (const [sign, word] of Object.entries(CURRENCY)) {
        ...
        s = rewrite(s, new RegExp(`${S}\\s?(\\d[\\d.,]*)((?:\\s+(?:${MAG_WORD})${GEO}*)?)` +
        ...
        s = rewrite(s, new RegExp(`(\\d[\\d.,]*)\\s${S}(?!\\d)`, "gu"), (_m, d: string) => `${figureToWords(d)} ${word}`);
    }
    ...
    for (const [pat, expansion] of ABBREV) {
        ...
        s = rewrite(s, new RegExp(`${NOT_LETTER_BEFORE}${pat}\\s*(?=[\\p{L}\\d])`, "gu"), `${expansion} `);
        s = rewrite(s, new RegExp(`${NOT_LETTER_BEFORE}${pat}(?=\\s*(?:[.,;:!?»)\\]]|$))`, "gu"), `${expansion}.`);
    }
    ...
    s = rewrite(s, new RegExp(`(?<![\\d.,/])(\\d{1,3})\\s?\\/\\s?(\\d{1,3})(?![\\d.,/])(?!\\s*(?:${MONTHS}))${DEG}`, "gu"),
    ...
    s = rewrite(s, new RegExp(`${NOT_LETTER_BEFORE}(?<![\\d.,])(\\d[\\d.,]*\\d|\\d)-(${WRITTEN_ALT})${NOT_LETTER_AFTER}`, "gu"),
    ...
    s = rewrite(s, new RegExp(`${NOT_LETTER_BEFORE}(?<![\\d.,])(\\d+)-(${GEO}{2,}იან[ია]?)${NOT_LETTER_AFTER}`, "gu"),
