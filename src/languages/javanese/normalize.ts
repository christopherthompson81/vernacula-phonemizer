...
export function normalizeJavanese(input: string): string {
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])Rp\.(?=\s*\d)/gu, "Rp");
    ...
    s = rewrite(s, /(?<![\d.,])(\d+),(\d{1,2})(?![\d,])/gu, (_m, i: string, f: string) => decimal(i, f));
    ...
    s = rewrite(s, new RegExp(`(?<!${L}|[\\d/])1/2(?!${L}|[\\d/])`, "gu"), "setengah");
    s = rewrite(s, new RegExp(`(?<!${L}|[\\d/])1/3(?!${L}|[\\d/])`, "gu"), "sapratelon");
    s = rewrite(s, new RegExp(`(?<!${L}|[\\d/])1/4(?!${L}|[\\d/])`, "gu"), "saprapat");
    ...
    s = rewrite(s, /(\d+)\s*%\s*[-–]\s*(?=\d)/gu, "$1% nganti ");
    s = rewrite(s, /(['’"”′″°])\s*[-–]\s*(?=\d)/gu, "$1 nganti ");
    ...
    s = rewrite(s, /(\d+)\s*°\s*C(?![\p{L}])/gui, "$1 drajat celsius");
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])(?:±|\+\/-)\s*(?=\d)/gu, "kurang luwih ");
