...
export function normalizeShonaPost(input: string): string {
    ...
    s = rewrite(s, /(?<![\d.,])(\d+[.,]\d+)[ \u00a0]?m([²³])?(?![\p{L}\p{M}'’ʼ\d])/gu,  // space, NBSP
    ...
    s = rewrite(s, /(?<![\d.,:\p{L}\p{M}])(\d+(?:[.,]\d+)?)[ \u00a0]?hrs?(?![\p{L}\p{M}\d])/gu, "maawa $1");  // space, NBSP
    ...
    s = rewrite(s, new RegExp(`(?<![\\d.,])(\\d+(?:[.,]\\d+)?)${DEG}[ \u00a0]?([CF])(?![\\p{L}\\p{M}])`, "gui"),  // space, NBSP
        (w, n: string, c: string, off: number, full: string) =>
            `${degreeBody(n, off, off + w.length, full)} ${SCALE[c.toUpperCase()]!}`);
    s = rewrite(s, new RegExp(`(?<![\\d.,])(\\d+(?:[.,]\\d+)?)${DEG}(?![\\p{L}\\p{M}])`, "gu"),
    ...
    s = rewrite(s, new RegExp(`((?:${MA_NOUNS})[ \u00a0])(\\d+)(?![.,]?\\d)`, "gu"),  // space, NBSP
    ...
    s = rewrite(s, /(?<![\d.,])(\d+)\.(\d+)(?![\d.,])/gu, (_m, i: string, f: string) => spell(i, POINT_WORD, f));
    s = rewrite(s, /(?<![\d.,])(\d+),(\d{1,2})(?![\d,])/gu, (_m, i: string, f: string) => spell(i, COMMA_WORD, f));
