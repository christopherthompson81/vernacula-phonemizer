...
export function normalizeShan(input: string): string {
    ...
    s = rewrite(s, new RegExp(`(?<![\\p{L}\\p{M}])A\\.?\\s?D\\.?${NOT_LETTER_AFTER}`, "gu"), "ပီၶရိတ်ႉ");
    ...
    s = rewrite(s, /(?<![\d:.,])([01]?\d|2[0-4]):([0-5]\d)(?![\d:.,])(?!\s*[မ][ူွ]င်း)/gu, (_m, h: string, mi: string) =>
    ...
    s = rewrite(s, /(\d)\s?°\s?(\d+)\s?['′]/gu, "$1 ၻီႇၵရီႇ $2 မိၼိတ်ႉ ");
    s = rewrite(s, /(\d)\s?°\s?[CF](?![\p{L}\p{M}])/gui, "$1 ၻီႇၵရီႇ ");
    s = rewrite(s, /(\d)\s?°/gu, "$1 ၻီႇၵရီႇ ");
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])(?:US|AU|CA|NZ|HK|SG)\s?(?=[$])/gu, "");
