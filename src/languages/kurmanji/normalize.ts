...
export function normalizeKurmanji(input: string): string {
    ...
    s = rewrite(s, group, (m) => m.replaceAll(".", "").replaceAll(",", ""));
    ...
    s = rewrite(s, /(?<![\p{L}])ji\s+(?=%\s?\p{Nd})/giu, "");
    ...
    s = rewrite(s, new RegExp(TEMP + "C(?![\\p{L}])", "giu"),
        (_m, sg: string, n: string) => `${neg(sg)}${n} pile Selsiyus`);
    s = rewrite(s, new RegExp(TEMP + "F(?![\\p{L}])", "giu"),
    ...
    s = rewrite(s, new RegExp(String.raw`([-−]?)(\p{Nd}+(?:[.,]\p{Nd}+)?)\s*°(?!\s*\p{L}(?!\p{L}))`, "gu"),
        (_m, sg: string, n: string) => `${neg(sg)}${n} pile`);
    ...
    s = rewrite(s, /(?<![\p{Nd}.,])(\p{Nd}+),(\p{Nd}{1,2})(?![\p{Nd}]|[.,]\p{Nd})/gu, (_m, w: string, f: string) => spell(w, f));
    s = rewrite(s, /(?<![\p{Nd}.,])(\p{Nd}+)\.(\p{Nd})(?![\p{Nd}]|[.,]\p{Nd})/gu, (_m, w: string, f: string) => spell(w, f));
