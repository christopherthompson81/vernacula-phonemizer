...
export function normalizeLithuanian(input: string): string {
    ...
    t = t.replace(new RegExp(`(?<=\\d)${SP}*x${SP}*(?=\\d)`, "gu"), " ");
    ...
    t = t.replace(new RegExp(`(^|[\\s(\\[])[-−–](?=\\d)`, "gu"), `$1${W.minus} `);
    ...
    t = t.replace(new RegExp(`(^|[\\s(\\[])\\+(?=\\d)`, "gu"), `$1${W.plus} `);
    ...
    for (const [sign, forms, spelled] of CURRENCIES) {
        const money = (num: string, mag: string | undefined, tail: string): string => {
            const abbrev = mag !== undefined && /^[\s\u00A0]*(?:mlrd|mln|tūkst)/u.test(mag);
    ...
    t = t.replace(new RegExp(`${NUM}${SP}*a\\.`, "gu"), (_m, num: string) => `${bare(num)} ${W.centuryGen} `);
    //     `val.` ×5 (hours), `min.` ×1 and `mėn.` ×3 (month), each after a numeral or a month name.
    t = t.replace(new RegExp(`${NUM}${SP}*val\\.`, "gu"), (_m, num: string) => quantity(num, N.hour));
    ...
    t = t.replace(new RegExp(`${NUM}${SP}*min\\.`, "gu"), (_m, num: string) => quantity(num, N.minute));
    ...
    t = t.replace(/&amp;/gu, "&").replace(/\s*&\s*/gu, ` ${W.and} `);
