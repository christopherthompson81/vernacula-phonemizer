...
export function normalizeLuganda(input: string): string {
    ...
    input = rewrite(input, CLOCK_MARKED, (m, h1?: string, m1?: string, h2?: string, m2?: string) =>
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])(\d+(?:\.\d+)?)[ \u00a0]?%/gu, (w, n: string, off: number, all: string) =>  // space, NBSP
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])US[ \u00a0]?\$[ \u00a0]?(?=\d)/giu, (w, off: number, all: string) =>  // space, NBSP
        NAMED_DOLLAR.test(all.slice(Math.max(0, off - 45), off + w.length + 45)) ? "" : `${DOLLAR} `);
    s = rewrite(s, /\$[ \u00a0]?(?=\d)/gu, (w, off: number, all: string) =>  // space, NBSP
        NAMED_DOLLAR.test(all.slice(Math.max(0, off - 45), off + w.length + 45)) ? "" : `${DOLLAR} `);
    s = rewrite(s, /€[ \u00a0]?(?=\d)/gu, (w, off: number, all: string) =>  // space, NBSP
    ...
    s = rewrite(s, /(?<![\d.,\p{L}\p{M}])(\d+(?:\.\d+)?)[ \u00a0\u202f\u2009]?km[²2](?![\p{L}\p{M}\d])/gu, km2);  // space, NBSP, NNBSP, thin space
    ...
    s = rewrite(s, /(?<![\d.,\p{L}\p{M}])(\d+(?:\.\d+)?)[ \u00a0\u202f\u2009]?km(?![\p{L}\p{M}\d²³/])/gu,  // space, NBSP, NNBSP, thin space
    ...
    s = rewrite(s, /(?<![\d.,\p{L}\p{M}])(\d+)[ \u00a0]m(?![\p{L}\p{M}\d²³/])/gu,  // space, NBSP
        (w, n: string, off: number, all: string) =>
    ...
    s = rewrite(s, /(?<![\d.,])(\d+)\.(\d+)(?![\d,\p{L}\p{M}]|\.\d)/gu,
