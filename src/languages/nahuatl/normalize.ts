...
export function normalizeNahuatl(input: string): string {
    ...
    s = rewrite(s, /(?<![\d:.,])([01]?\d|2[0-3]):([0-5]\d):([0-5]\d)(?![\d:.,])/gu, "$1 $2 $3");
    ...
    s = rewrite(s, new RegExp(`(?<![\\d:.,])([01]?\\d|2[0-3]):([0-5]\\d)(?![\\d:.,])(\\s?)hrs${NOT_LETTER_AFTER}`, "gu"),
        "$1 $2$3horas");
    ...
    s = rewrite(s, /(\d)\s?°\s?C(?![\p{L}\p{M}])/gui, "$1 grados");
    s = rewrite(s, /(\d)\s?°/gu, "$1 grados ");
    ...
    for (const [abbr, word] of UNITS) {
        s = rewrite(s, new RegExp(`(\\d)\\s+${abbr}(?![\\p{L}\\p{M}\\d²³/])`, "gu"), `$1 ${word}`);
    }
