...
export function normalizeKamba(input: string): string {
    ...
    s = rewrite(s, new RegExp(`${NOT_BEFORE}(\\d[\\d,.]*)\\s?sq\\s?mi${NOT_AFTER}`, "gu"), "sikwea sya maili $1");
    ...
    s = rewrite(s, new RegExp(`${NOT_BEFORE}(\\d[\\d,.]*)\\s?mph${NOT_AFTER}`, "gu"), "maili $1 kwa isaa");
    ...
    s = rewrite(s, new RegExp(`${NOT_BEFORE}[Nn]dola\\s+((?:US|AUD)?\\$)\\s?(?=\\d)`, "gu"), "$1");
    ...
    s = rewrite(s, new RegExp(`(\\d[\\d.,]*)\\s?°\\s?[CF]?${NOT_AFTER}`, "gui"),
