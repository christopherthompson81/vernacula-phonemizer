...
export function normalizeSundanese(input: string): string {
    ...
    s = rewrite(s, /(?<![\d.,])([1-9]\d{0,2}(?:[ \u00a0\u202f\u2009]\d{3})+)(?!\d)/gu, (m) => m.replace(/[ \u00a0\u202f\u2009]/gu, ""));  // space, NBSP, NNBSP, thin space
    ...
    s = rewrite(s, /(?<![\d.:])([01]?\d|2[0-3]):([0-5]\d)\b(?!\.?\d)/gu,
    ...
    s = rewrite(s, /(?<=[\p{L}\p{M}])\/(km|m|cm|mm|kg|g|ha|l|c)(²|³|2|3)?(?![\p{L}\p{M}\d])/giu,
        (_m, u: string, exp: string | undefined) => ` per ${UNIT_WORD[u.toLowerCase()] ?? u}${EXP_WORD[EXP_ASCII[exp ?? ""] ?? exp ?? ""] ?? ""}`);
    s = rewrite(s, /\bper\s+(km|m|cm|mm|kg|g|ha|l)(²|³|2|3)?(?![\p{L}\p{M}\d])/giu,
    ...
    s = rewrite(s, /(\d)[.,](\d{1,2})(?![\d.,])/gu, (_m, a: string, b: string) => `${a} koma ${[...b].join(" ")}`);
    ...
    s = rewrite(s, /(\d+(?:-an)?)\s*SM\b(?![\p{L}\p{M}])/gu, "$1 saméméh Maséhi");
    ...
    s = rewrite(s, /(\d+(?:-an)?)\s*M\b(?![\p{L}\p{M}]|\.\p{L})/gu, "$1 Maséhi");
    ...
    s = rewrite(s, /(?<![\d/])(\d{4})\/(\d{4})(?![\d/])/gu, (m0, a: string, b: string) =>
    ...
    s = rewrite(s, /(\d)\s?°\s?C(?![\p{L}\p{M}])/giu, "$1 darajat Celsius");
    s = rewrite(s, /(\d)\s?°\s?F(?![\p{L}\p{M}])/giu, "$1 darajat Fahrenheit");
    ...
    s = rewrite(s, /(\d)\s?°(?=[\p{L}\p{M}])/gu, "$1 darajat ");
    s = rewrite(s, /(\d)\s?°/gu, "$1 darajat");
    ...
    s = rewrite(s, /(\S)\+\s?(\(?\s?[-−]?\d)/gu, "$1 tambah $2");
    s = rewrite(s, /(^|\s)\+\s?(\(?\s?[-−]?\d)/gu, "$1tambah $2");
    ...
    s = rewrite(s, /(^|[\s(])[-−–](\d)/gu, "$1kurang $2");
    s = rewrite(s, /\s?=\s?/gu, " sarua jeung ");
    s = rewrite(s, /\s?<\s?/gu, " leuwih leutik ti ");
    s = rewrite(s, /\s?>\s?/gu, " leuwih gedé ti ");
