...
export function normalizeIndonesian(input: string): string {
    ...
    s = rewrite(s, /(?<![\d.:])([01]?\d|2[0-3])[.:]([0-5]\d)\b(?!\.?\d)/gu, (whole, h: string, min: string) => {
        const mv = Number(min);
    ...
    s = rewrite(s, new RegExp(`(\\d)\\s?(${UNIT_ALT})(?![\\p{L}])`, "gu"),
    ...
    s = rewrite(s, /(\d)\s?°\s?C(?![\p{L}\p{M}])/giu, "$1 derajat Celsius");
    s = rewrite(s, /(\d)\s?°\s?F(?![\p{L}\p{M}])/giu, "$1 derajat Fahrenheit");
    ...
    s = rewrite(s, /\s?=\s?/gu, " sama dengan ");
    s = rewrite(s, /\s?<\s?/gu, " lebih kecil dari ");
    s = rewrite(s, /\s?>\s?/gu, " lebih besar dari ");
