...
export function normalizeSinhala(input: string): string {
    ...
    for (const [rx, word] of DOTTED_ABBREV) s = rewrite(s, rx, word);
    ...
    s = rewrite(s, /(?<![඀-෿.])(?:[඀-෿]{1,7}\.){2,}/gu, (m) => m.replaceAll(".", " ").trimEnd() + " ");
    ...
    s = rewrite(s, new RegExp(SIGN + NUM + DEG + "C(?![\\p{L}\\p{M}])", "giu"),
        (_m, sg: string, n: string) => `සෙල්සියස් අංශක ${neg(sg)}${n}`);
    s = rewrite(s, new RegExp(SIGN + NUM + DEG + "F(?![\\p{L}\\p{M}])", "giu"),
    ...
    s = rewrite(s, new RegExp(NUM + DEG + "([NSEW])(?![\\p{L}\\p{M}])", "gu"),
        (_m, n: string, d: string) => `අංශක ${n} ${COMPASS[d]}`);
    s = rewrite(s, new RegExp(SIGN + NUM + "\\s*[°º\\u2070]", "gu"),
    ...
    s = rewrite(s, new RegExp(NUM + " K(?![\\p{L}\\p{M}.])", "gu"), "කෙල්වින් $1");
    ...
    s = rewrite(s, /(^|[(\[])[-–−](?=\p{Nd})/gu, "$1ඍණ ");
    s = rewrite(s, /(?<=[\s(\[])−(?=\p{Nd})/gu, "ඍණ ");
    ...
    s = rewrite(s, /((?:US\$|[$€£₨])\s?\p{Nd}[\p{Nd}.,]*)m(?![\p{L}\p{M}])/gu, "$1 මිලියන");
    ...
    s = rewrite(s, /(?<![\p{L}\p{Nd}.,])(\p{Nd}+\.\p{Nd}+)m(?![\p{L}\p{M}])/gu, "මීටර් $1");
    s = SYMBOLS(s);
