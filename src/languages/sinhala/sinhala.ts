/**
 * Sinhala (si) phonemizer — canonical IPA. The systematic abugida parsing is done by the
 ...
 * inherent-vowel schwa alternation (a in the first syllable of a polysyllable, else ə).
 */
...
function applySchwa(ipa: string): string {
    ...
    for (const m of ipa.matchAll(VOWEL_G)) {
        ...
        vi++;
        if (m[0] === "a") {
            const atEnd = m.index + 1 === ipa.length;
            out += vi === 0 && !atEnd ? "a" : "ə";
        } else out += m[0];
...
for (const cls of MANIFEST.anusvara.classes)
    for (const c of cls.triggers) NASAL_CODA[c] = cls.nasal;
/** Rewrite each ං to its homorganic nasal coda (default ම් = m). */
function anusvara(word: string): string {
    return word.replace(
        /ං(.?)/gu,
        (_m, nxt: string) =>
            (NASAL_CODA[nxt] ?? MANIFEST.anusvara.default) + nxt,
    );
}
...
export function phonemizeWord(word: string): string {
    ...
    for (let vi = 0; vi < nuclei.length; vi++) {
        ...
        out +=
            (vi === 0
                ? "ˈ"
                : vi >= 2 && vi % 2 === 0 && vi !== nuclei.length - 1
                  ? "ˌ"
                  : "") + m[0];
        i = m.index! + m[0].length;
    }
...
export function createSinhala(): Phonemizer {
    return new SinhalaPhonemizer();
}
