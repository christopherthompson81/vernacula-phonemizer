...
function eraMarker(text: string): string {
    return rewrite(text, new RegExp(`${NOT_LETTER_BEFORE}p\\s?\\.\\s?Kr\\s?\\.`, "gu"),
...
export function normalizeLatgalian(input: string): string {
    ...
    s = rewrite(s, new RegExp(`${NOT_LETTER_BEFORE}u\\.c\\.`, "gu"),
        (m0, off: number, full: string) => others(m0, off, full, "i cyti"));
    s = rewrite(s, new RegExp(`${NOT_LETTER_BEFORE}ct\\.`, "gu"),
    ...
    s = rewrite(s, /(?<=\d)(\s*)(?:mln|mil)\s?\./gu, (_m, gap: string) => `${gap || " "}milijoni`);
    ...
    s = rewrite(s, /(?<![\d.,])(\d+),(\d+)(?!\d)/gu, (_m, head: string, frac: string) => {
