...
export function normalizeYoruba(text: string): string {
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}\p{Nd}])(?<!\p{Nd}\s)\u2212(?=\p{Nd})/gu, `${SYM.negative} `);
    ...
    s = rewrite(s, /\(\s*(\d+(?:\.\d+)?)\s*%\s*\)/gu, (m, num: string, at: number, whole: string) =>
        SAID_AFTER.test(fold(whole.slice(Math.max(0, at - 60), at))) ? "" : m,
    );
    s = rewrite(s, PERCENT, (_m, num: string, at: number, whole: string) =>
    ...
    s = rewrite(s, UNIT_SQUARED, (_m, u: string) => `${UNIT_WORDS[u] ?? u} ${SYM.squared}`);
    ...
    s = rewrite(s, METRE, (_m, num: string) => `${num} ${METRE_WORD}`);
    ...
    s = rewrite(s, DECIMAL, (_m, whole: string, frac: string) => `${whole} ${SYM.decimalWord} ${[...frac].join(" ")}`);
