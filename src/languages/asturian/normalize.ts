...
export function normalizeAsturian(input: string): string {
    ...
    s = rewrite(s, new RegExp(`${NOT_LETTER_BEFORE}(\\d{1,2})\\s?-\\s?(I{1,3}|IV|V|VI{1,3}|IX|XI{0,2})\\s?-\\s?(\\d{3,4})${NOT_LETTER_AFTER}`, "gu"),
    ...
    s = rewrite(s, new RegExp(`${NOT_LETTER_BEFORE}(\\d{1,2})\\s?-\\s?(\\d{1,2})\\s?-\\s?(\\d{3,4})${NOT_LETTER_AFTER}`, "gu"),
    ...
    s = rewrite(s, /(\d)\s?[°º]\s?(\d+)\s?[′']/gu, "$1 graos $2 minutos ");
    s = rewrite(s, new RegExp(`(\\d)\\s?[°º](?=${DEGREE_TAIL})`, "gu"), "$1 graos ");
