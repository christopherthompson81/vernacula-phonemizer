...
export function normalizeAbkhaz(text: string): string {
    ...
    s = rewrite(s, /(?<![\d,])[1-9]\d{0,2}(?:,\d{3}){2,}(?![\d,])/gu, (m) => m.replace(/,/gu, ""));
    s = rewrite(s, /(?<![\d,.])([1-9]\d{0,2}),(\d{3})(?![\d,])/gu, "$1$2");
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}\p{Nd}])(?<!\p{Nd}\s)\u2212(?=\p{Nd})/gu, `${MANIFEST.symbols.minus} `);
    ...
    s = rewrite(s, new RegExp(`(?<![\\d:])${TIME}\\s?[-–—]\\s?${TIME}(?![\\d:]|[.,]\\d)`, "gu"),
    ...
    s = rewrite(s, new RegExp(`(?<![\\d:])${TIME}(?![\\d:]|[.,]\\d)`, "gu"),
    ...
    s = rewrite(s, new RegExp(`${HOUR_ANCHOR}${DTIME}(?!\\d|[.,]\\d)`, "gu"),
    ...
    s = rewrite(s, /(\d)\s?°\s?[CС](?![\p{L}])/gui, (_m, d: string) => `${d} ${MANIFEST.symbols.celsius}`);
    s = rewrite(s, /(\d)\s?°(?!\s?[CFKКСФ](?![\p{L}]))/gui, (_m, d: string) => `${d} ${MANIFEST.symbols.degree}`);
    ...
    for (const [abbr, slot] of Object.entries(MANIFEST.symbols.scales))
        s = rewrite(s, new RegExp(`(?<![\\p{L}])${ESC(abbr)}(?:\\.(\\s+\\p{Lu})?|(?![\\p{L}]))`, "gu"),
    ...
    s = rewrite(s, /(?<=\d\s?(?:км|см|м))([23])(?![\d\p{L}])/gu, (d: string) => (d === "2" ? "²" : "³"));
    ...
    //    sentence dot.
    s = rewrite(s, /(?<![\p{L}\d.,-])(\d+(?:[.,]\d+)?)\s?[-–—]\s?(\d+(?:[.,]\d+)?)(?!\d|[.,]\d|-)/gu,
    ...
    s = rewrite(s, /(?<![\d.])(\d+)[.,](\d+)(?!\d|\.\d)/gu, (m0, int: string, frac: string, off: number, whole: string) => {
    ...
    //    ахәбатәижәа.
    s = rewrite(s, new RegExp(`(\\d+)[- ]${MANIFEST.numbers.ordinalSuffix}(?![\\p{L}])`, "gu"),
    ...
    for (const [abbr, full] of MANIFEST.abbreviations)
        ...
        s = rewrite(s, new RegExp(`(?<![\\p{L}])${abbr.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&")}\\.(\\s+\\p{Lu})?`, "gu"),
