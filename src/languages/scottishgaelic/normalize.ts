...
export function normalizeScottishGaelic(input: string): string {
    ...
    s = rewrite(s, new RegExp(`${NOT_BEFORE}(${ABBREV_ALT})\\.(\\s+)(?=[\\p{L}\\d(])`, "giu"),
    ...
    s = rewrite(s, new RegExp(`${NOT_BEFORE}(${ABBREV_ALT})\\.(?=\\s*(?:[.,;:!?»)]|$))`, "giu"),
        (m0, ab: string) => {
            // ⚠ THE MISS BRANCH IS REACHABLE (#1122): the pattern is built from this table's own
            // keys but carries `i`+`u`, so JS's fold widens it and a near-miss matches while its
            // key is absent. The `!` here made `String.replace` stringify `undefined`.
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])(?:no|àir)\.\s?(?=\d)/giu, "àireamh ");
    s = rewrite(s, /№\s?(?=\d)/gu, "àireamh ");
    ...
    s = rewrite(s, /(?<![\d.,])((?:1\d|20)\d0)s(?![\p{L}\p{M}])/gu, "$1");
    ...
    s = rewrite(s, new RegExp(`(?<![\\d.,])(\\d{1,2})-?(mh|na|s|d)(?:(\\s+)(${WORD}))?${NOT_LETTER}`, "giu"),
    ...
    s = rewrite(s, /(?<![\d.,])(\d+)\.(\d{1,2})(?![\d.\p{L}])/gu,
