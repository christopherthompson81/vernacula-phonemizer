...
export function normalizeTelugu(input: string): string {
    ...
    s = rewrite(s, /(?<![\d.,])(1[1-9]\d{2})(?![\d]|\.\d)/gu, (_m, y: string) => yearToWords(Number(y)));
    ...
    s = rewrite(s, new RegExp(`${NOT_LETTER_BEFORE}ఉదా\\s*\\.\\s*(?=[\\p{L}])`, "gu"), "ఉదాహరణకు ");
    ...
    s = rewrite(s, KM_RE, "కిలోమీటర్లు");
    s = rewrite(s, MI_RE, "మైళ్లు");
    ...
    s = rewrite(rewrite(s, /(?<=\d)\s?½/gu, ".5"), /(?<=\d)\s?¾/gu, ".75");
    ...
    s = rewrite(s, /\s?=\s?/gu, " సమానం ");
    s = rewrite(s, /\s?÷\s?/gu, " భాగించడం ");

    s = rewrite(s, /(\d)\s?°\s?C(?![\p{L}])/giu, "$1 డిగ్రీల సెల్సియస్");
    s = rewrite(s, /(\d)\s?°\s?F(?![\p{L}])/giu, "$1 డిగ్రీల ఫారెన్‌హీట్");  // ZWNJ
    s = rewrite(s, /(\d)\s?°/gu, "$1 డిగ్రీలు");
