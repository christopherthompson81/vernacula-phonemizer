...
export function normalizeCantonese(input: string, measureWords: string): string {
    ...
    s = rewrite(s, /\s*=\s*/gu, " 等於 ");
    s = rewrite(s, /\s*<\s*/gu, " 小於 ");
    s = rewrite(s, /\s*>\s*/gu, " 大於 ");
    s = rewrite(s, /\s*÷\s*/gu, " 除以 ");
    ...
    s = rewrite(s, /(?<![\d.,:])(\d{4})(?![\d.,])(?=\s*年)/gu, (_m, y: string) => spellDigits(y));
