...
export function normalizeSomali(input: string): string {
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])(\d+)H(?![\p{L}\p{M}])/gu, "$1 Hijri");
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])(\d+)M(?![\p{L}\p{M}])/gu, (whole, n: string, off: number, src: string) => {
    ...
    s = rewrite(s, /(?<![\d.:])([01]?\d|2[0-3]):([0-5]\d)(?!:\d)(?!\.?\d)(?:(?=\s?[AaPp]\.?[Mm]\.?)|\b)/gu,
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])(\d+)(?:st|nd|rd|th)(?![\p{L}\p{M}])/giu, "$1aad");
    ...
    s = rewrite(s, new RegExp(String.raw`(?<![\p{L}\p{M}])(${UNIT_KEYS})\s+([23])(?![\d\p{L}\p{M}])`, "gu"), "$1$2");
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])(\d)\s*mph(?![\p{L}\p{M}])/gu, "$1 mi/h");
    ...
    s = rewrite(s, new RegExp(String.raw`(\d)-(?=(?:${UNIT_KEYS})(?:[²³23])?(?![\p{L}\p{M}\d]))`, "gu"), "$1 ");
    ...
    s = rewrite(s, /(\d)[.,](\d{1,2})(?![\d.,])/gu, (_m, a: string, b: string) => `${a} dhibic ${[...b].join(" ")}`);
    ...
    s = rewrite(s, /(\d)\s*C\.?\s?H\.?(?![\p{L}\p{M}])/gu, "$1 Ciise Hortiis");
    ...
    s = rewrite(s, /(\d)\s*C\.?\s?D\.?(?![\p{L}\p{M}])/gu, "$1 Ciise Dabadiis");
    s = rewrite(s, /(\d)\s*(?:BCE|BC)(?![\p{L}\p{M}])/gu, "$1 Ciise Hortiis");
    s = rewrite(s, /(\d)\s*(?:CE|AD)(?![\p{L}\p{M}])/gu, "$1 Miilaadi");
    s = rewrite(s, /(\d)\s*AH(?![\p{L}\p{M}])/gu, "$1 Hijri");
    // ── 7. RANGES → `ilaa` ("up to") ───────────────────────────────────────────────────────────────────
    ...
    s = rewrite(s, /(\d)\s?°\s?C(?![\p{L}\p{M}])/giu, "$1 darajo Celsius");
    s = rewrite(s, /(\d)\s?°\s?F(?![\p{L}\p{M}])/giu, "$1 darajo Fahrenheit");
    s = rewrite(s, /(\d)\s?°\s?([NSEW])(?![\p{L}\p{M}])/giu,
    ...
    s = rewrite(s, /(\S)\+\s?(\(?\s?[-−]?\d)/gu, "$1 ku dar $2");
    s = rewrite(s, /(^|\s)\+\s?(\(?\s?[-−]?\d)/gu, "$1ku dar $2");
    s = rewrite(s, /(^|[\s(])[-−–](\d)/gu, "$1laga jaray $2");
    // ⚠ `±` IS ONE CHARACTER (U+00B1) and no `+` rule can match inside it — ×22, and it needs its own arm.
    s = rewrite(s, /±/gu, " ku dar ama laga jaray ");
    s = rewrite(s, /\s?=\s?/gu, " u dhiganta ");
    s = rewrite(s, /\s?<\s?/gu, " ka yar ");
    s = rewrite(s, /\s?>\s?/gu, " ka badan ");
    s = rewrite(s, /\s?÷\s?/gu, " loo qeybiyay ");
