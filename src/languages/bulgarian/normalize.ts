...
export function normalizeBulgarian(input: string): string {
    ...
    t = tr(t, /пр\.\s*н\.\s*е\./giu, "преди новата ера");
    t = tr(t, /сл\.\s*н\.\s*е\./giu, "след новата ера");
    ...
    t = tr(t, /(?<!\p{L})км\s*\/\s*ч(?!\p{L})/giu, "километра в час");
    t = tr(t, /(\p{L}+)\s*\/\s*час(?!\p{L})/giu, "$1 в час");
    ...
    t = tr(t, /(?<!\p{L})м\s*\/\s*(?:сек|с)(?!\p{L})/giu, "метра в секунда");
    ...
    t = tr(t, /(?<!\p{L})km\s*\/\s*h(?!\p{L})/giu, "километра в час");
    t = tr(t, /(?<!\p{L})m\s*\/\s*s(?!\p{L})/giu, "метра в секунда");
    ...
    for (const [abbr, word] of UNITS)
        t = tr(t, new RegExp(`(\\d)\\s*${abbr}(?!\\p{L})`, "gu"), `$1 ${word}`);

    // 11) RANGES, spoken `до`.
    t = tr(t, /(?<![-–—])(\d+)\s*[-–—]\s*(\d+)(?!\d)(?!\s*[-–—]\s*\d)/gu, "$1 до $2");
