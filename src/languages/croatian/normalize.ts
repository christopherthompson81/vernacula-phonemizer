...
export function normalizeCroatian(input: string): string {
    ...
    s = rewrite(s, /(\d{1,4})\.\s*[-–—]\s*(\d{1,4})\.(?=\s*(?:n\.\s?e\.|p\.\s?n\.\s?e\.))/gu, "$1 do $2");
    ...
    s = rewrite(s, /(?<![\d.,])(\d{1,4})\.\s+(?:g\.\s+)?(?=(?:n\.\s?e|p\.\s?n\.\s?e|pr\.\s?Kr\.)(?![\p{L}\p{M}]))/gu,
        (whole, digits: string) => {
            const base = ordinalBase(Number(digits));
            return base === undefined ? whole : `${inflect(base, "f.gen")!} `;
        });
    s = rewrite(s, /(?<![\p{L}\p{M}])p\.\s?n\.\s?e\.(?=[.!?]|$)/gu, "prije nove ere.");
    s = rewrite(s, /(?<![\p{L}\p{M}])p\.\s?n\.\s?e\.(\s)/gu, "prije nove ere$1");
    s = rewrite(s, /(?<![\p{L}\p{M}])n\.\s?e\.(?=[.!?]|$)/gu, "nove ere.");
    s = rewrite(s, /(?<![\p{L}\p{M}])n\.\s?e\.(\s)/gu, "nove ere$1");
    s = rewrite(s, /(?<![\p{L}\p{M}])pr\.\s?Kr\.(?=[.!?]|$)/gu, "prije Krista.");
    s = rewrite(s, /(?<![\p{L}\p{M}])pr\.\s?Kr\.(\s)/gu, "prije Krista$1");
    // The `g.` in `400. g. n. e.` / `1000. g. pr. Kr.` is "godine" (elided); drop it after the claim.
    s = rewrite(s, /(?<=\d)\s+g\.\s+(?=(?:n\.\s?e\.|pr\.\s?Kr\.))/gu, " ");

    // 3) DOTTED ABBREVIATIONS. `itd.` → "i tako dalje". The dot is consumed before a following word.
    s = rewrite(s, /(?<![\p{L}\p{M}])itd\.(\s+)(?=[\p{L}\d(])/giu, "i tako dalje$1");
    s = rewrite(s, /(?<![\p{L}\p{M}])itd\.(?=\s*[,;:])/giu, "i tako dalje");
    s = rewrite(s, /(?<![\p{L}\p{M}])itd\.(?=\s*(?:[.!?”"»)\]])|$)/giu, "i tako dalje.");
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])Dr\.(?=\s*(?:[.,;:!?»)]|$))/giu, "Doktor.");
    s = rewrite(s, /(?<![-\p{L}\p{M}])SAD(?=-)/gu, "Sjedinjene Američke Države");
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])([IVXL]+)\.\s+(\p{Ll}[\p{L}\p{M}]*)/gu,
    ...
    s = rewrite(s, /(\d+)\s?°\s?([CFcf])(?![\p{L}\p{M}])/gui, (_m, n: string, u: string) =>
    ...
    s = rewrite(s, /(\d+)\s?°(?:\s?([NEWSnew])|([s]))(?![\p{L}\p{M}])/gu, (_m, n: string, spaced: string | undefined, tight: string | undefined) =>
    ...
    s = rewrite(s, /(\d+)\s?°(?:\s?[QXYqxy](?![\p{L}\p{M}]))?/gu,
    ...
    s = rewrite(s, /(?<![\d:.,])([01]?\d|2[0-3]):([0-5]\d)(?![:.\d])(?:\s*h)?/giu,
    ...
    s = rewrite(s, /(?<!\p{L}\p{M})(\p{Lu})&(\p{Lu})(?![^\p{L}\p{M}])/gu, "$1 i $2");
    s = rewrite(s, /\s&\s/gu, " i ");
    ...
    s = rewrite(s, /(?<=[A-Z])\+(\d)/gu, " plus $1");
    s = rewrite(s, /(?<![\p{L}\p{Nd}])[−-](\d+)(?!\s*[-–—\d])/gu, "minus $1");
    s = rewrite(s, /(\S)\s*=\s*(\S)/gu, "$1 jednako $2");
    s = rewrite(s, /(\d)\s*<\s*(\d)/gu, "$1 manje od $2");
    s = rewrite(s, /(\d)\s*>\s*(\d)/gu, "$1 veće od $2");
    ...
    s = rewrite(s, /(\S)\s*÷\s*(\S)/gu, "$1 podijeljeno s $2");
