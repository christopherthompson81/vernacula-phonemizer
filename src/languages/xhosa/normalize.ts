...
export function normalizeXhosa(input: string): string {
    ...
    //    "kunye amp ;".
    s = rewrite(rewrite(s, /&amp;/giu, "&"), /&/gu, " kunye ");
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])(?:\p{Lu}\.){2,}/gu, (run, off: number, full: string) => {
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])(\p{Lu})\.(\p{Lu})(?![\p{L}\p{M}])/gu, "$1$2");
    ...
    s = rewrite(s, /(?<![\p{Lu}\p{M}])(\p{Lu})\.(?=\p{Lu}\p{Ll})/gu, "$1 ");
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])([uU]?)Mnu\.?(?=[ \u00a0]\p{Lu})/gu, "$1Mnumzana");  // space, NBSP
    s = rewrite(s, /(?<![\p{L}\p{M}])Jr\.(?=[ \u00a0]\p{Ll})/gu, "Jr");  // space, NBSP
    ...
    s = rewrite(s, /(?<![\d.,])([1-9]\d{0,2})((?:[ \u00a0\u202f\u2009]\d{3})+)(?![\d])/gu,  // space, NBSP, NNBSP, thin space
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])(US|AUD)[ \u00a0]+(?=[$£¥€][ \u00a0]?\d)/gu, "$1");  // space, NBSP
    ...
    s = rewrite(s,
        ...
        (whole, sym: string, int: string, sep: string, frac: string, mag: string) =>
            decimal(sep, frac)
                ...
                : whole);
    s = rewrite(s, /(?<![\d.,])(\d+)([.,])(\d+)[ \u00a0]?(km|mm|cm|kg|mi|m)(?![\p{L}\p{M}'’ʼ])/gu,  // space, NBSP
    ...
    s = rewrite(s, /(?<![\d.,])(\d+\.\d+)[ \u00a0]?[-–][ \u00a0]?(\d+\.\d+)(?![\d.])/gu, "$1 ukuya ku $2");  // space, NBSP
    ...
    s = rewrite(s, /(?<![\d:.,])([01]?\d|2[0-3]):[ \u00a0]?([0-5]\d)(?![:.\d])(?:[ \u00a0]*([AaPp])\.?[Mm]\.?(?![\p{L}\p{M}]))?/gu,  // space, NBSP
    ...
    s = rewrite(s, /(?<![\d.,])(\d{1,2})\.([0-5]\d)(?![.\d])[ \u00a0]*(UTC|GMT)/gu,  // space, NBSP
    ...
    s = rewrite(s, /(\d)[ \u00a0]?:[ \u00a0]?(?=\d)/gu, "$1 ");  // space, NBSP
    ...
    s = rewrite(s, /(?<![\d.,])(\d+)[ \u00a0]?[-–][ \u00a0]?(\d+)(?![\d.,])/gu,  // space, NBSP
    ...
    s = rewrite(s, /(?<![\d.,])(\d+)[ \u00a0]?(km|mi|m)[ \u00a0]*\/[ \u00a0]*(h|u|s)(?![\p{L}\p{M}])/giu,  // space, NBSP
    ...
    s = rewrite(s, /(?<![\d.,])(\d+)[ \u00a0]?(mph|kph)(?![\p{L}\p{M}])/giu,  // space, NBSP
    ...
    s = rewrite(s, /(\d+)[ \u00a0]?°[ \u00a0]?([NSEW])(?![\p{L}\p{M}])/gu, (_m, n: string, c: string, off: number, full: string) =>  // space, NBSP
        `${saidBefore(full, off, "maqondo") ? n : `amaqondo ${n}`} ${COMPASS[c]!}`);
    s = rewrite(s, /(\d+)[ \u00a0]?[°º](?![\p{L}\p{M}])/gu, (_m, n: string, off: number, full: string) =>  // space, NBSP
    ...
    s = rewrite(s, /[ \u00a0]*[=≈][ \u00a0]*/gu, " lilingana ne ");  // space, NBSP
    s = rewrite(s, /[ \u00a0]*<[ \u00a0]*/gu, " ngaphantsi kuna ");  // space, NBSP
    s = rewrite(s, /[ \u00a0]*>[ \u00a0]*/gu, " ngaphezulu kuna ");  // space, NBSP
    s = rewrite(s, /(\d)[ \u00a0]*×[ \u00a0]*(?=\d)/gu, "$1 phindaphinda ");  // space, NBSP
    s = rewrite(s, /[ \u00a0]*÷[ \u00a0]*/gu, " yahlula ");  // space, NBSP
    ...
    s = rewrite(s, /(?<=[\p{L}\d])\+(?=\d)/gu, " plas ");
    s = rewrite(s, /(?<![\p{L}\p{M}\d])\+[ \u00a0]?(?=\d)/gu, "plas ");  // space, NBSP
    s = rewrite(s, /(?<![\p{L}\p{M}\d])(?<![\p{L}\p{M}][ \u00a0])[-−](?=\d)/gu, "thabatha ");  // space, NBSP
    ...
    //     swallowed.
    s = rewrite(s, /(?<![\d.,])(\d+)\.(\d+)(?![\d])/gu, (_m, int: string, frac: string) => spell(int, frac));
    s = rewrite(s, /(?<![\d.,])(\d+),(\d{1,2})(?![\d,])/gu, (_m, int: string, frac: string) => spell(int, frac));
