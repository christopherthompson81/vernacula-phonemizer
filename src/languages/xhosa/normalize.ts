...
export function normalizeXhosa(input: string): string {
    ...
    s = tr(s, /(?<![\p{L}\p{M}])(?:\p{Lu}\.){2,}/gu, (run, off: number, full: string) => {
    ...
    s = tr(s, /(?<![\p{L}\p{M}])(\p{Lu})\.(\p{Lu})(?![\p{L}\p{M}])/gu, "$1$2");
    ...
    s = tr(s, /(?<![\p{Lu}\p{M}])(\p{Lu})\.(?=\p{Lu}\p{Ll})/gu, "$1 ");
    ...
    s = tr(s, /(?<![\p{L}\p{M}])([uU]?)Mnu\.?(?=[ \u00a0]\p{Lu})/gu, "$1Mnumzana");  // space, NBSP
    s = tr(s, /(?<![\p{L}\p{M}])Jr\.(?=[ \u00a0]\p{Ll})/gu, "Jr");  // space, NBSP
    ...
    s = tr(s, /(?<![\p{L}\p{M}])(US|AUD)[ \u00a0]+(?=[$£¥€][ \u00a0]?\d)/gu, "$1");  // space, NBSP
    ...
    s = tr(s,
        ...
        (whole, sym: string, int: string, sep: string, frac: string, mag: string) =>
            decimal(sep, frac)
                ...
                : whole);
    s = tr(s, /(?<![\d.,])(\d+)([.,])(\d+)[ \u00a0]?(km|mm|cm|kg|mi|m)(?![\p{L}\p{M}'’ʼ])/gu,  // space, NBSP
    ...
    s = tr(s, /(?<![\d.,])(\d+\.\d+)[ \u00a0]?[-–][ \u00a0]?(\d+\.\d+)(?![\d.])/gu, "$1 ukuya ku $2");  // space, NBSP
    ...
    s = tr(s, /(?<![\d:.,])([01]?\d|2[0-3]):[ \u00a0]?([0-5]\d)(?![:.\d])(?:[ \u00a0]*([AaPp])\.?[Mm]\.?(?![\p{L}\p{M}]))?/gu,  // space, NBSP
    ...
    s = tr(s, /(?<![\d.,])(\d{1,2})\.([0-5]\d)(?![.\d])[ \u00a0]*(UTC|GMT)/gu,  // space, NBSP
    ...
    s = tr(s, /(?<![\d.,])(\d+)[ \u00a0]?(km|mi|m)[ \u00a0]*\/[ \u00a0]*(h|u|s)(?![\p{L}\p{M}])/giu,  // space, NBSP
    ...
    s = tr(s, /(?<![\d.,])(\d+)[ \u00a0]?(mph|kph)(?![\p{L}\p{M}])/giu,  // space, NBSP
    ...
    s = tr(s, /(\d+)[ \u00a0]?°[ \u00a0]?([NSEW])(?![\p{L}\p{M}])/gu, (_m, n: string, c: string, off: number, full: string) =>  // space, NBSP
        `${saidBefore(full, off, "maqondo") ? n : `amaqondo ${n}`} ${COMPASS[c]!}`);
    s = tr(s, /(\d+)[ \u00a0]?[°º](?![\p{L}\p{M}])/gu, (_m, n: string, off: number, full: string) =>  // space, NBSP
    ...
    s = tr(s, /[ \u00a0]*[=≈][ \u00a0]*/gu, " lilingana ne ");  // space, NBSP
    s = tr(s, /[ \u00a0]*<[ \u00a0]*/gu, " ngaphantsi kuna ");  // space, NBSP
    s = tr(s, /[ \u00a0]*>[ \u00a0]*/gu, " ngaphezulu kuna ");  // space, NBSP
    s = tr(s, /(\d)[ \u00a0]*×[ \u00a0]*(?=\d)/gu, "$1 phindaphinda ");  // space, NBSP
    s = tr(s, /[ \u00a0]*÷[ \u00a0]*/gu, " yahlula ");  // space, NBSP
    ...
    s = tr(s, /(?<=[\p{L}\d])\+(?=\d)/gu, " plas ");
    s = tr(s, /(?<![\p{L}\p{M}\d])\+[ \u00a0]?(?=\d)/gu, "plas ");  // space, NBSP
    s = tr(s, /(?<![\p{L}\p{M}\d])(?<![\p{L}\p{M}][ \u00a0])[-−](?=\d)/gu, "thabatha ");  // space, NBSP
    ...
    //     swallowed.
    s = tr(s, /(?<![\d.,])(\d+)\.(\d+)(?![\d])/gu, (_m, int: string, frac: string) => spell(int, frac));
    s = tr(s, /(?<![\d.,])(\d+),(\d{1,2})(?![\d,])/gu, (_m, int: string, frac: string) => spell(int, frac));
