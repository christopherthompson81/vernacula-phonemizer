...
export function normalizeMongolian(input: string): string {
    ...
    s = rewrite(s, INITIALS, (_m, run: string) =>
    ...
    s = rewrite(s, DIGIT_COLON_RUN, (_m, head: string, rest: string) => head + rest.replace(COLON_G, " "));

    // 4. Ordinals. Above the percent and unit steps only because nothing else spends a hyphen before a ⟨р⟩.
    s = rewrite(s, ORDINAL, (m, digits: string, tail: string) => {
    ...
    s = rewrite(s, CURRENCY_BEFORE, (m, sign: string, num: string, mag: string, off: number, whole: string) =>
        currency(m, num, sign, mag, off, whole));
    s = rewrite(s, CURRENCY_AFTER, (m, num: string, sign: string, mag: string, off: number, whole: string) =>
        currency(m, num, sign, mag, off, whole));
    ...
    s = rewrite(s, DEGREE_SCALE, (m, digit: string, scale: string) =>
    ...
    s = rewrite(s, UNIT, (_m, num: string, key: string, exp: string | undefined, suffix: string | undefined) => {
