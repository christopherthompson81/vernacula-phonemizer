...
export function normalizeZulu(input: string): string {
    ...
    s = s.replace(/(?<=\d[ \u00a0])BC(?![\p{L}\p{M}])/gu, BCE_WORD);  // space, NBSP
    ...
    s = s.replace(/(?<=\p{L}[ \u00a0])(\p{Lu})\.(?=[ \u00a0]+\p{Lu})/gu, "$1");  // space, NBSP
    ...
    s = s.replace(/(?<![\d.,])(\d{1,2})[.,]([0-5]\d)(?=[ \u00a0]*(?:UTC|GMT)(?![\p{L}\p{M}]))/gu,  // space, NBSP
        (_m, h: string, min: string) => clockBody(h, min));
    s = s.replace(/(?<![\d.,])(0\d)([0-5]\d)(?=[ \u00a0]*(?:UTC|GMT)(?![\p{L}\p{M}]))/gu,  // space, NBSP
        (_m, h: string, min: string) => clockBody(h, min));
    ...
    s = s.replace(/[ \u00a0]?\+[ \u00a0]?(?=\d)/gu, " plas ");  // space, NBSP
    ...
    s = s.replace(/(\d[\d.,]*)[ \u00a0]?[°º][ \u00a0]?C(?![\p{L}\p{M}])/gui,  // space, NBSP
    ...
    s = s.replace(/(?<![\d.,])(\d+\.\d+)[ \u00a0]*[-–—][ \u00a0]*(\d+\.\d+)(?![\d.,])/gu, "$1 kuya ku-$2");  // space, NBSP
    s = s.replace(/(?<![\d.,])(\d[\d,]*\d|\d)[ \u00a0]*[-–—][ \u00a0]*(\d[\d,]*\d|\d)(?![\d.,])/gu, span);  // space, NBSP
    ...
    s = s.replace(/(?<!\d)(\d[\d.,]*)[ \u00a0]?(km|mi|m|mm|cm|kg)[ \u00a0]*\/[ \u00a0]*([hs])(?![\p{L}\p{M}])/giu,  // space, NBSP
    ...
    s = s.replace(/(?<![\p{L}\p{M}])(?:US\$|AUD\$|\$|£)[ \u00a0]?(\d+)\.(\d+)(?!\.?\d)/gu,  // space, NBSP
        (_m, i: string, f: string) => `${dec(i, f)} amadola`);
    s = s.replace(/(?<![\d.,])(\d+)\.(\d+)[ \u00a0]?(km|mi|mm|cm|kg|ft|m)([²2])?(?![\p{L}\p{M}'’])/giu,  // space, NBSP
    ...
    s = s.replace(/(?<![\d/.,])1[ \u00a0]?\/[ \u00a0]?(\d{1,2})(?![\d/.,])/gu, (m0, d: string) => {  // space, NBSP
    ...
    s = s.replace(/[ \u00a0]?×[ \u00a0]?/gu, " kuphindwe ngo-");  // space, NBSP
    s = s.replace(/[ \u00a0]?=[ \u00a0]?/gu, " kulingana no-");  // space, NBSP
    s = s.replace(/[ \u00a0]?<[ \u00a0]?/gu, " ngaphansi kuka-");  // space, NBSP
    s = s.replace(/[ \u00a0]?>[ \u00a0]?/gu, " ngaphezu kuka-");  // space, NBSP
    ...
    s = s.replace(/(?<![\p{L}\p{Nd}])(?<![\p{L}\p{Nd}][ \u00a0])[-−](?=\d)/gu, "ukukhipha ");  // space, NBSP
