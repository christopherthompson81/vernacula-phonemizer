...
export function normalizeKinyarwanda(input: string): string {
    ...
    s = rewrite(s, new RegExp(`(?<![\\d.,])[1-9]\\d{0,2}(?:,\\d{3})+(?!\\d|[.,]\\d)${NOT_COORD}`, "gu"), (w) => w.replace(/,/gu, ""));
    s = rewrite(s, new RegExp(`(?<![\\d.,])[1-9]\\d{0,2}(?:\\.\\d{3})+(?!\\d|[.,]\\d)${NOT_COORD}`, "gu"), (w) => w.replace(/\./gu, ""));
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}\d])[-−–][ \u00a0]?(\d+(?:[.,]\d+)?)[ \u00a0]?°[ \u00a0]?([CF])(?![\p{L}\p{M}])/gui,
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}\d.,-])(\d+)[ \u00a0]?[-–—][ \u00a0]?(\d+)[ \u00a0]?°[ \u00a0]?([CF])?(?![\p{L}\p{M}])/gui,  // space, NBSP
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])(\d+(?:[.,]\d+)?)[ \u00a0]?°((?:[ \u00a0]?\d+[′'][ \u00a0]?)?(?:\d+[″"][ \u00a0]?)?)[ \u00a0]?([NSEW])(?![\p{L}\p{M}])/gu,  // space, NBSP
    ...
    s = rewrite(s, new RegExp(`(?<![\\d:])(\\d{1,2}):(\\d{2}):(\\d{2})(?=[ \u00a0]*(?:${TZ})(?![\\p{L}\\p{M}]))`, "gu"), "$1 $2 $3");  // space, NBSP
    ...
    s = rewrite(s, /(?<![\d:.])(\d{1,2}):[ \u00a0]?(\d{2}):[ \u00a0]?(\d{2})(?![:.\d])/gu,  // space, NBSP
    ...
    s = rewrite(s, /(?<![\d:])(\d{1,2}):[ \u00a0]?(\d{2})(?![:\d])/gu, "$1 $2");  // space, NBSP
    ...
    s = rewrite(s, /(?<![\d.,])(\d+)[ \u00a0]?%[ \u00a0]?[-–—][ \u00a0]?(\d+)[ \u00a0]?%/gu,  // space, NBSP
