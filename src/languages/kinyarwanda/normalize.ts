...
export function normalizeKinyarwanda(input: string): string {
    ...
    s = s.replace(/(?<![\p{L}\p{M}])(\d+(?:[.,]\d+)?)[  ]?°((?:[  ]?\d+[′'][  ]?)?(?:\d+[″"][  ]?)?)[  ]?([NSEW])(?![\p{L}\p{M}])/gu,
    ...
    s = s.replace(
        new RegExp(`(?<![\\p{L}\\p{M}\\d])(${HOUR_MARKER})[  ]+([01]?\\d|2[0-3]):[  ]?([0-5]\\d)(?![:.\\d])`, "giu"),
    ...
    s = s.replace(new RegExp(`(?<![\\d:])(\\d{1,2}):(\\d{2}):(\\d{2})(?=[  ]*(?:${TZ})(?![\\p{L}\\p{M}]))`, "gu"), "$1 $2 $3");
    ...
    s = s.replace(/(?<![\d:.])(\d{1,2}):[  ]?(\d{2}):[  ]?(\d{2})(?![:.\d])/gu,
    ...
    s = s.replace(
        new RegExp(`(?<![-\\d.,\\p{L}\\p{M}])(\\d+)[  ]?[-–—][  ]?(\\d+)[  ]?(${RANGE_UNIT})(²|³|(?<=[a-zA-Z])[23](?![\\d\\p{L}]))?(?![\\p{L}\\p{M}\\d'’])`, "gu"),
