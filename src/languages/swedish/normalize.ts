...
export function normalizeSwedish(input: string): string {
    ...
    t = rewrite(t, /(?<![\d:,])(\d{1,2}):(\d{2})[.,](\d{1,2})(?!\d)/gu, "$1 $2,$3");
    ...
    t = rewrite(t, /(?<![\d:,])(?<!\.\d)(\d{1,2})\.(\d{2})(?![\d.])/gu, (m, h: string, mm: string) =>
    ...
    t = rewrite(t, /(?<![\d:,])(\d{1,2}):(\d{2})(?![\d.,])/gu, (m, h: string, mm: string) =>
        isClock(h, mm) ? `${h} ${minutes(mm)}` : m);
    ...
    t = rewrite(t, /(?<![\d:.,])(\d{1,2}):(\d{1,2})(?![\d\p{L}])/gu, "$1 $2");
    ...
    for (const [re, word, keepFinal] of ABBREV)
        t = rewrite(t, re, (m: string, ...rest: unknown[]) => {
    ...
    t = rewrite(t, /(?<![\d:.,])(\d{1,3}):(a|e|s|or)(?![\p{L}\p{M}])/gu, (m, n: string, suf: string) => {
    ...
    t = rewrite(t, /(?<![\d.,:])(\d{3,4})-(tal\p{L}*)/gu, (m, y: string, tail: string) => {
    ...
    t = rewrite(t, /(?<![\d.,:\p{L}])(1[1-9]\d\d)(?![\d,:])(?!\.\d)(?!\p{L})(?!\s*(?:%|[$€£°²³]|(?:km|cm|mm|kg|[Gg][Hh][Zz]|[Mm][Bb][Ii][Tt]|m)(?![\p{L}\p{M}])))/gu, (m, y: string) =>
    ...
    t = rewrite(t, /(\d)\s*°\s*C(?![\p{L}\p{M}])/gui, "$1 grader celsius");
    t = rewrite(t, /(\d)\s*°\s*F(?![\p{L}\p{M}])/gui, "$1 grader fahrenheit");
    t = rewrite(t, /(\d)\s*°\s*V(?![\p{L}\p{M}])/gu, "$1 grader väst");
    ...
    t = rewrite(t, /(?<=[\d\p{L}])\s+x\s+(?=\d)/gu, " gånger ");
    ...
    t = rewrite(t, /(?<![\p{L}\p{M}\d])([-−+])(\d)/gu, (_m, sign: string, d: string) =>
        `${sign === "+" ? "plus" : "minus"} ${d}`);
    t = rewrite(t, /(?<=[\p{L}\p{M}])\+(?=\d)/gu, " plus ");
