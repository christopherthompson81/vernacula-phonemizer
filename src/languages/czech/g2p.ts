...
const VOWEL: Record<string, string> = {
  ...
  "á": "aː", "é": "ɛː", "í": "iː", "ó": "oː", "ú": "uː", "ů": "uː", "ý": "iː",
};
const PALAT_TRIGGER = new Set(["i", "í", "ě"]); // d/t/n palatalise before these
const PALAT: Record<string, string> = { "d": "ɟ", "t": "c", "n": "ɲ" };
const CONS: Record<string, string> = {
  "b": "b", "c": "t͡s", "č": "t͡ʃ", "d": "d", "ď": "ɟ", "f": "f", "g": "ɡ", "h": "ɦ", "j": "j", "k": "k",
  "l": "l", "m": "m", "n": "n", "ň": "ɲ", "p": "p", "q": "k", "r": "r", "ř": "r̝", "s": "s", "š": "ʃ",
  "t": "t", "ť": "c", "v": "v", "w": "v", "x": "ks", "z": "z", "ž": "ʒ",
};

// Voicing-assimilation pairs (both directions), keyed by IPA phoneme.
const TO_VOICELESS: Record<string, string> = { "b": "p", "d": "t", "ɟ": "c", "ɡ": "k", "v": "f", "z": "s", "ʒ": "ʃ", "ɦ": "x", "d͡z": "t͡s", "d͡ʒ": "t͡ʃ", "r̝": "r̝̊" };
const TO_VOICED: Record<string, string> = { "p": "b", "t": "d", "c": "ɟ", "k": "ɡ", "f": "v", "s": "z", "ʃ": "ʒ", "x": "ɦ", "t͡s": "d͡z", "t͡ʃ": "d͡ʒ", "r̝̊": "r̝" };
...
function markSyllabic(segs: Seg[]): void {
  for (let i = 0; i < segs.length; i++) {
    ...
    if (!leftV && !rightV) { s.ph = s.ph === "r" ? "r̩" : "l̩"; s.nucleus = true; }
  }
}
...
function applyVoicing(segs: Seg[]): void {
  for (let i = segs.length - 1; i >= 0; i--) {
    ...
    else if (nx.ph === "ɦ") target = "voiceless";               // before ɦ a voiced obstruent devoices (rozhodně→rosɦodɲɛ)
    else if (isObstruent(nx.ph) && nx.ph !== "v" && nx.ph !== "r̝" && nx.ph !== "r̝̊") { // v and ř do not trigger
  ...
  for (let i = 1; i < segs.length; i++) {
    if (segs[i]!.ph === "r̝" && segs[i - 1]!.ph in TO_VOICED) segs[i]!.ph = "r̝̊";
  }
}
...
function degeminate(segs: Seg[]): void {
  for (let i = segs.length - 1; i > 0; i--) {
    ...
    if (a.ph === "n" && (b.ph === "n" || b.ph === "ɲ")) segs.splice(i - 1, 1);
  }
}
