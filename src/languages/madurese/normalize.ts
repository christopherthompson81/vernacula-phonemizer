...
export function normalizeMadurese(input: string): string {
    ...
    s = rewrite(s, /(?<![\d.:,])([01]?\d|2[0-3]):00(?![\d.:])/gu, (_m, h: string) => String(Number(h)));
    ...
    s = rewrite(s, /(?<![\d.,])(?!0,)[1-9]\d{0,2}(?:,\d{3})+(?![\d,])/gu, (m) => m.replaceAll(",", ""));
    ...
    s = rewrite(s, /(?<=\d)(['’′″"”º°]+)\s*[-–—]\s*(?=\d)/gu, "$1 sampè' ");
    ...
    s = rewrite(s, /(\d)\s?[°º]\s?C(?![\p{L}\p{M}])/gui, "$1 derajat celcius");
    s = rewrite(s, /(\d)\s?[°º]\s?F(?![\p{L}\p{M}])/gui, "$1 derajat fahrenheit");
    s = rewrite(s, /(\d)\s?[°º]\s?/gu, "$1 derajat ");
    ...
    s = rewrite(s, /((?:korang lebbi|ra-kèra|sakètar|kèra-kèra)\s?)±\s*(?=\d)/gu, "$1");
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}\d])(\d+)\s?SM(?![\p{L}\p{M}\d])/gu, "$1 sabellunna Masèhi");
    s = rewrite(s, /(?<![\p{L}\p{M}\d])(\d+)\s?M(?![\p{L}\p{M}\d])/gu, "$1 Masèhi");
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])(m[èe]t[èe]r)\s+dpl(?![\p{L}\p{M}\d])/giu, "$1 è attas tasè'");
    ...
    s = rewrite(s, /(?<![\d.,])(\d+),(\d{1,3})(?![\d,])/gu, (_m, i: string, f: string) => decimal(i, f));
