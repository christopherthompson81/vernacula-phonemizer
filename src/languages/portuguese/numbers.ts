...
export function numberToWords(n: number): string {
    ...
    if (n < 1e6) {
        const th = Math.floor(n / 1000),
            r = n % 1000;
        const thousand =
            th === 1 ? N.thousand : `${below1000(th)} ${N.thousand}`;
        ...
        return r < 100 || r % 100 === 0
            ? `${thousand} ${E} ${below1000(r)}`
            : `${thousand} ${below1000(r)}`;
    }
