...
export function normalizeWestArmenian(input: string): string {
    ...
    s = s.replace(new RegExp(`(\\d[^\\s]*)\\s?ա\\s?\\.\\s?մ\\s?\\.`, "gu"), "$1 աստղագիտական միաւոր");
    ...
    s = s.replace(new RegExp(`(\\d)\\s?°\\s?[CС]\\s?-\\s?(${ARM_LOWER}+)${NOT_AFTER}`, "gui"), "$1 սելսիուս աստիճան$2");
    s = s.replace(new RegExp(`(\\d)\\s?°\\s?F\\s?-\\s?(${ARM_LOWER}+)${NOT_AFTER}`, "gui"), "$1 ֆարենհայթ աստիճան$2");
    ...
    s = s.replace(/(^|(?<!\d)[\s(])[-−–]\s?(\d)/gu, "$1մինուս $2");
    ...
    s = s.replace(/(\d)\s?=\s?(?=\d)/gu, "$1 հաւասար ");
