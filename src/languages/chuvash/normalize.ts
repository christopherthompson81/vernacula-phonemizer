...
export function normalizeChuvash(input: string): string {
    ...
    s = rewrite(s, new RegExp(`(?<![\\p{L}\\p{M}])млрд\\.?${NOT_LETTER_AFTER}`, "giu"), "миллиард");
    s = rewrite(s, new RegExp(`(?<![\\p{L}\\p{M}])трлн\\.?${NOT_LETTER_AFTER}`, "giu"), "триллион");
    s = rewrite(s, new RegExp(`(?<![\\p{L}\\p{M}])млн\\.?${NOT_LETTER_AFTER}`, "giu"), "миллион");
    ...
    s = rewrite(s, new RegExp(`(?<![\\p{L}\\p{M}])п\\.\\s?эрч\\.`, "giu"), "пирӗн эраччен");
    s = rewrite(s, new RegExp(`(\\d)\\s?ҫҫ\\.`, "gu"), "$1 ҫулсем");
    s = rewrite(s, new RegExp(`(\\d)\\s?ҫ\\.`, "gu"), "$1 ҫул");
    ...
    s = rewrite(s, /(?<![\d:.,])([01]?\d|2[0-4]):([0-5]\d):([0-5]\d|60)(?![\d:.,])/gu,
        (_m, h: string, mi: string, sec: string) =>
            `${cardinal(Number(h))} ${cardinal(Number(mi))} ${cardinal(Number(sec))}`);
    s = rewrite(s, /(?<![\d:.,])([01]?\d|2[0-4]):\s?([0-5]\d)(?![\d:.,])/gu, (_m, h: string, mi: string) => {
        const mv = Number(mi);
    ...
    s = rewrite(s, new RegExp(`(?<![\\d.,/])(\\d{1,2})\\s?/\\s?(\\d{1,2})(?=\\s(?:пай|пая|пайĕ|пайӗ)${WORD}*)`, "gu"),
    ...
    s = rewrite(s, new RegExp(`(?<![\\d.,])(\\d+)\\s?-\\s?(\\d+)\\s?-\\s?(м[ĕӗ]ш${SFX}{0,8})${NOT_LETTER_AFTER}`, "gu"),
    ...
    s = rewrite(s, new RegExp(`(?<![\\d.,/])(\\d+)\\s?-\\s?(м[ĕӗ]ш${SFX}{0,6})${NOT_LETTER_AFTER}`, "gu"),
    ...
    s = rewrite(s, /(\d)\s?°\s?[CСс](?![\p{L}\p{M}])/gui, "$1 Цельси градусӗ");
    s = rewrite(s, /(\d)\s?°\s?[FФф](?![\p{L}\p{M}])/gui, "$1 Фаренгейт градусӗ");
...
export function spellAttributive(input: string): string {
    return rewrite(input, new RegExp(`(?<![\\d.,:/-])(\\d+)(\\s)(?=${WORD})`, "gu"), (whole, digits: string, gap: string) => {
        const n = Number(digits);
        if (!Number.isSafeInteger(n)) return whole;
