...
export function normalizeSylheti(input: string): string {
    ...
    s = s.replace(new RegExp(`(?:${S}+\\.)+${S}+\\.?`, "gu"), (m) => m.replace(/\./gu, ""));
    ...
    s = s.replace(new RegExp(`(?<![\\p{Nd}.,])(${NUM})\\s*[-–—]\\s*(${NUM})(?![\\p{Nd}.,])`, "gu"),
    ...
    s = s.replace(/(?<!\.\p{Nd}*)(\p{Nd})\.(?=\p{Nd})(?!\p{Nd}*[./])/gu, `$1 ${DECIMAL_WORD} `);
    ...
    s = s.replace(/(\p{Nd})\s*%/gu, `$1 ${PERCENT_WORD}`);
    s = s.replace(/%/gu, ` ${PERCENT_WORD} `);
    ...
    s = s.replace(/৳\s*(\p{Nd}+)(?![^\p{Nd}]{0,3}ꠐꠦꠈꠣ)/gu, `$1 ${CURRENCY_WORD}`);
    s = s.replace(/৳\s*(\p{Nd}+)/gu, "$1");
    ...
    s = s.replace(/৳(?![^\p{Nd}]{0,3}ꠐꠦꠈꠣ)/gu, ` ${CURRENCY_WORD} `);
    s = s.replace(/৳/gu, "");
