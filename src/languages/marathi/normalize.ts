...
export function makeMarathiNormalizer(def: MarathiWords): (text: string) => string {
    ...
    return (input: string): string => {
        ...
        s = rewrite(rewrite(s, /अ[‌‍]?ॅ/gu, "ऍ"), /अ[‌‍]?ॉ/gu, "ऑ");  // ZWNJ, ZWJ
        s = rewrite(s, /[‌‍]/gu, "");  // ZWNJ, ZWJ
        ...
        s = rewrite(s, /(?<![\p{L}\p{M}])[इई]\.?\s?स\.?\s?प[ूु]\.?/gu, ERA.bc);
        s = rewrite(s, /(?<![\p{L}\p{M}])[इई]\.?\s?प[ूु]\.?/gu, ERA.bc);
        s = rewrite(s, /(?<![\p{L}\p{M}])[इई]\.?\s?स\./gu, ERA.ad);
        ...
        s = rewrite(s, /(?<![\p{L}\p{M}])डॉ\.?(\s+)(?=[\p{L}])/gu, `${def.abbreviations["डॉ"]}$1`);
        ...
        s = rewrite(s, /(?<![\d.,:])(\d+)(\s*)(?=व[ाीे])/gu, (whole, d: string, sp: string) => {
            const n = Number(d);
        ...
        s = rewrite(s, /(\d)\s?°\s?(?:C|से\.?)(?![\p{L}\p{M}])/giu, `$1 ${DEG.word} ${DEG.celsius}`);
        s = rewrite(s, /(\d)\s?°\s?F(?![\p{L}\p{M}])/giu, `$1 ${DEG.word} ${DEG.fahrenheit}`);
        s = rewrite(s, /(\d)\s?°\s?N(?![\p{L}\p{M}])/gu, `$1 ${DEG.word} ${DEG.north}`);
        s = rewrite(s, /(\d)\s?°\s?S(?![\p{L}\p{M}])/gu, `$1 ${DEG.word} ${DEG.south}`);
        s = rewrite(s, /(\d)\s?°\s?E(?![\p{L}\p{M}])/gu, `$1 ${DEG.word} ${DEG.east}`);
        s = rewrite(s, /(\d)\s?°\s?W(?![\p{L}\p{M}])/gu, `$1 ${DEG.word} ${DEG.west}`);
        s = rewrite(s, /(\d)\s?°/gu, `$1 ${DEG.word}`);
        ...
        //    the singular.
        s = rewrite(s, /(\d+(?:[.,]\d+)*)\s?[%٪％]/gu, (_m, n: string) =>
        ...
        s = rewrite(s, /(?<![\d,.\-–—])100(?![\d,.\-–—])(?!\s*[A-Za-z])/gu, def.bareHundred);
        ...
        s = rewrite(s, /(?<![\p{L}\p{M}\p{Nd}])−\s?(?=\d)/gu, ` ${SIGN.minus} `);  // U+2212 MINUS SIGN
        s = rewrite(s, /~\s?(?=\d)/gu, ` ${SIGN.approximately} `);
