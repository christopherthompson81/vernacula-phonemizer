...
export function normalizeSlovenian(input: string): string {
    ...
    for (const [re, w] of MULTI_DOT) s = rewrite(s, re, (m0: string, ...rest: unknown[]) => keepFinal(w, m0, rest));
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}.])(Inc)\.(?=\s+\p{Ll})/gu, "$1");
    s = rewrite(s, /(?<=(?:et|Et)\s)(al)\.(?![\p{L}\p{M}])/gu, "$1");
    ...
    s = rewrite(s, /(?<![\d.,])(\d+(?:,\d+)*)\s?[–—-]\s?(\d+(?:,\d+)*)(?![\d.,])/gu, (m0, a: string, b: string) =>
    ...
    //    rather than guessed.
    s = rewrite(s, /(?<![\d.,\p{L}\p{M}])(\d{1,4})\s?-\s?ih(?![\p{L}\p{M}])/gu, (m0, digits: string) => {
    ...
    s = rewrite(s, /(\d)-(?=(?:km|mm|cm|kg|m)(?![\p{L}\p{M}]))/gu, "$1 ");
    ...
    s = rewrite(s, /(\d+(?:,\d+)?)\s?°/gu, (_m, n: string) => `${n} ${counted(numOf(n), STOPINJA())}`);
    ...
    s = rewrite(s, /(?<![\d.,-])(\d+)-(\p{Ll}{4,})/gu, (m0, n: string, tail: string) => {
        const v = Number(n);
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])milj\s?\/\s?(?:uro|h)(?![\p{L}\p{M}])/giu, "milj na uro");
    ...
    for (const c of Object.values(COUNTED)) {
        ...
        if (c.g === "m") {
            s = rewrite(s, new RegExp(`(?<![\\d.,])1 (?=${esc(sg!)}(?![\\p{L}\\p{M}]))`, "gu"), "en ");
            ...
            s = rewrite(s, new RegExp(`(?<![\\d.,])(?<!do )3 (?=${esc(paucal!)}(?![\\p{L}\\p{M}]))`, "gu"), "trije ");
            s = rewrite(s, new RegExp(`(?<![\\d.,])(?<!do )4 (?=${esc(paucal!)}(?![\\p{L}\\p{M}]))`, "gu"), "štirje ");
        } else {
            s = rewrite(s, new RegExp(`(?<![\\d.,])2 (?=${esc(dual!)}(?![\\p{L}\\p{M}]))`, "gu"), "dve ");
        }
    }
    ...
    s = rewrite(s, /(?<=\d),(\d+)/gu, (_m, frac: string) => {
    ...
    s = rewrite(s, /(?<![\d.,\/])(\d{1,3})\/(\d{1,2})(?![\d.,\/])/gu, (m0, a: string, b: string) => frac(a, b, m0));
    ...
    s = rewrite(s, /(\d+)¾/gu, "$1 in tri četrtine");
    s = rewrite(s, /(\d+)½/gu, "$1 in pol");
    s = rewrite(s, /(\d+)¼/gu, "$1 in ena četrtina");

    // 18) RELATIONAL SIGNS and the AMPERSAND.
    s = rewrite(s, /\s*=\s*/gu, " enako ");
    s = rewrite(s, /(\d)\s*<\s*(?=\d)/gu, "$1 je manjše od ");
    s = rewrite(s, /(\d)\s*>\s*(?=\d)/gu, "$1 je večje od ");
    ...
    s = rewrite(s, /(?<![\p{L}\p{M}])(\p{Lu})\s*[&＆]\s*(\p{Lu})(?![\p{L}\p{M}])/gu, (m0, a: string, b: string) => {
    ...
    s = rewrite(s, /(?<=\d)(\p{L})(?![\p{L}\p{M}])/gu, (m0, l: string) => {
