...
export function normalizeSindhi(input: string): string {
    ...
    t = t.replace(/(\d{1,2})\.(\d{2})(?=\s*(?:GMT|UTC|وڳي|بجي))/gu, "$1 ڪلاڪ $2 منٽ");
    ...
    t = t.replace(/(\d{1,2}):(\d{2})(?!\d)/gu, "$1 ڪلاڪ $2 منٽ");
    ...
    t = t.replace(/(\d+)\s*%/gu, "$1 سيڪڙو");
    t = t.replace(/%\s*(\d+)/gu, "$1 سيڪڙو");
    ...
    t = t.replace(/(\d)\s*°\s*C(?!\p{L})/giu, "$1 ڊگري سينٽي گريڊ");
    t = t.replace(/(\d)\s*°\s*F(?!\p{L})/giu, "$1 ڊگري فارينهائيٽ");
    t = t.replace(/(\d)\s*°/gu, "$1 ڊگري");
    ...
    t = t.replace(/(?<![-–—])(\d+)\s*[-–—]\s*(\d+)(?!\d)(?!\s*[-–—]\s*\d)/gu, "$1 کان $2 تائين");
    ...
    t = t.replace(/±/gu, " جمع منفي ");
    t = t.replace(/(\d)\s*\+\s*(\d)/gu, "$1 جمع $2");
