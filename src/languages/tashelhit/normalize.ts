...
export function normalizeTashelhit(input: string): string {
    ...
    for (const [body, word] of ERA)
        s = rewrite(s, new RegExp(`(?<![\\p{L}\\p{M}])${body}(?![\\p{L}\\p{M}.])`, "gu"), word);
    ...
    //    a separate figure.
    s = rewrite(s, /(?<![\d.,])([1-9]\d{0,2})((?:,\d{3})+)(?![\d]|,\d)(?!\s?%)/gu, (w) => w.replace(/,/gu, ""));
    ...
    s = rewrite(s, /°(?=[Ss]ils)/gu, " taskflt n ");
    s = rewrite(s, /(\d)\s?°\s?C(?![\p{L}\p{M}])/gui, "$1 taskflt n Silsyus");
