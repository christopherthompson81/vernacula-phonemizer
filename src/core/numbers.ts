...
export interface NumbersDef {
  ...
  magnitudes: { hundred: string; thousand: string; lakh: string; crore: string };
...
export const indicNumberWords: NumberComposer = (n, d) => {
  ...
  if (n < 1000) { const h = Math.floor(n / 100), r = n % 100; return [d.units[h]!, d.magnitudes.hundred, ...(r ? indicNumberWords(r, d) : [])]; }
  if (n < 100000) { const th = Math.floor(n / 1000), r = n % 1000; return [...indicNumberWords(th, d), d.magnitudes.thousand, ...(r ? indicNumberWords(r, d) : [])]; }
  if (n < 10000000) { const l = Math.floor(n / 100000), r = n % 100000; return [...indicNumberWords(l, d), d.magnitudes.lakh, ...(r ? indicNumberWords(r, d) : [])]; }
  const c = Math.floor(n / 10000000), r = n % 10000000;
  return [...indicNumberWords(c, d), d.magnitudes.crore, ...(r ? indicNumberWords(r, d) : [])];
};
...
export function renderNumber(n: number, d: NumbersDef, word: (w: string) => string, compose: NumberComposer = indicNumberWords): string {
  return compose(n, d).map((w) => (w === null ? "?" : word(w))).join(" ");
}
