...
interface Unit { text: string; isStress: boolean; isVowel: boolean }
...
function segmentUnits(ipa: string): Unit[] {
  ...
  while (i < ipa.length) {
    const c = ipa[i]!;
    if (c === "ˈ" || c === "ˌ") { units.push({ text: c, isStress: true, isVowel: false }); i++; continue; }
    let unit = c; i++;
    while (i < ipa.length) {
      const n = ipa[i]!;
      if (n === "͡") { unit += n + (ipa[i + 1] ?? ""); i += 2; continue; } // tie bar links next base
      if (/[̀-ͯ]/u.test(n) || MOD.has(n)) { unit += n; i++; continue; }     // combining / modifier
      break;
    }
    units.push({ text: unit, isStress: false, isVowel: HI_VOWEL_BASES.has(unit[0]!) });
  }
  return units;
}
...
export function deleteMedialSchwa(ipa: string): string {
  return ipa.split(/(\s+)/u).map((w) => {
    if (!/\S/.test(w)) return w;
    const units = segmentUnits(w);
    const deleted = new Array(units.length).fill(false);
    const prevPhon = (from: number): number => { let k = from; while (k >= 0 && (units[k]!.isStress || deleted[k])) k--; return k; };
    const nextPhon = (from: number): number => { let k = from; while (k < units.length && (units[k]!.isStress || deleted[k])) k++; return k; };
    for (let idx = units.length - 1; idx >= 0; idx--) {
      if (units[idx]!.isStress || deleted[idx] || units[idx]!.text !== "ə") continue;
      ...
      if (p >= 0 && pp >= 0 && n < units.length && nn < units.length &&
        !units[p]!.isVowel && units[pp]!.isVowel && !units[n]!.isVowel && units[nn]!.isVowel &&
        !units[p]!.text.includes("ː") && !units[n]!.text.includes("ː")) {
        deleted[idx] = true;
        if (idx - 1 >= 0 && units[idx - 1]!.isStress) deleted[idx - 1] = true; // stress was on the deleted schwa
      }
    }
    return units.filter((_, k) => !deleted[k]).map((u) => u.text).join("");
  }).join("");
}
