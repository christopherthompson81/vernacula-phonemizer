...
A **canonical-IPA** phonemizer for **142 languages** — native, cleanroom, and self-contained.
...
142 languages spanning Armenian, Austroasiatic,
...
| Germanic | Afrikaans `af`, Danish `da`, Dutch `nl`, English `en` (+ `en-GB`, `en-IN` accent variants), German `de`, Norwegian `nb` (Bokmål), Swedish `sv` (tonal accent), Luxembourgish `lb` |
| Hellenic | Greek `el` |
