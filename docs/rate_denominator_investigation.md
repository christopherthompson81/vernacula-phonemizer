...
| tl | `s: segundo`, `bawat` | `h` | `oras` ×16 mined / ×3 attest, in the hour sense (`mga ilang oras bago`, `ang yunit ng oras`) |
| an | `h: ora`, `por` | `s` | `metros cubicos por segundo` ×2, `un metro por segundo`, `radians por segundo (rad/s)` — the slot itself |
| haw | `h: hola`, `o ka` | `s` | `kekona` ×7 attest + ×1 mined, digit-adjacent (`60 kekona`, `90 kekona`, `He 30 kekona ka manawa`) |
