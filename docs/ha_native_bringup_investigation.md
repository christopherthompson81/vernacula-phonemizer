...
bring-up in vernacula — espeak ships nothing for Hausa; espeak-ng-portable authored it, so its authored output
...
## Convention (from espeak-ng-portable's authored ha_rules / ph_hausa)
...
Wiktionary/kaikki-derived, ported from espeak-ng-portable) — one code per vowel nucleus (H→˥ / L→˩ / F→˥˩ /
...
## Validation
vs the espeak-ng-portable AUTHORED gold (50k words): **exact 97.4%, segmental 97.4%**. The residual (~2.6%) is
