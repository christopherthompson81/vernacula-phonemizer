...
`docs/investigations/chv/chv_normalization_investigation.md`.
...
"a Latin-script host never prewarms" class in `docs/investigations/csharp-port/csharp_port_findings_investigation.md` — both engines keep the current behaviour).
...
- **The four sync-mode foreign-run differences are the English engine, not Chuvash, and both engines
  ...
  fleet-wide in `docs/investigations/csharp-port/csharp_port_findings_investigation.md`; a Latin-script host (chv is Cyrillic, but the embedded run is Latin) that
