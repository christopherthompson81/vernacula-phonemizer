...
gold resolves those to the later element. So the two sites keep different ones because they are
resolving different things, not because the engine is inconsistent. It is the one asymmetry in this
change and it is the only configuration with zero regressions and a real gain.
