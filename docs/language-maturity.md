...
| **cy** Welsh | 67.1% (.63) | — | 🟡 | Runs 1–4 (dialect-matched North Welsh referee). Run 4 fixed the **diphthong nuclei** against the referee (56.5→**67.1%**): final unstressed ⟨-au⟩→[a] (NW reduction: llyfrau→ɬəvra), ⟨wy⟩→ʊɨ (eglwys→ɛɡlʊɨs), ⟨ew⟩→ɛu, ⟨ei⟩/⟨ey⟩→ei (the big one, cymdeithas→…deiθas), ⟨eu⟩→eɨ. Remaining residual: **English/foreign-name code-switching** (Algeria→ald͡ʒiːrja — referee applies English phonology to English names), ⟨gwy⟩/⟨yw⟩ structural (gwyn→ɡwɨn) tangled with the y-obscure rule, ⟨ae⟩ (referee-contested: eɨ/ei/aᶤ), n/r/l length (folded away) — intricate/lexical. |
