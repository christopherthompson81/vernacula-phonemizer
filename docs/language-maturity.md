...
| **cy** Welsh | 76.2% (.73) | — | 🟡 | Runs 1–6 (North Welsh referee). Session **+19.7** by finding espeak-oracle artifacts + real rules: diphthong nuclei (final ⟨-au⟩→a, wy→ʊɨ, ei→ei), penult vowels stay **lax** (pobol→pɔbɔl — artifact, cf. i→ɨ), ⟨gwy⟩/⟨chwy⟩ structural, **double consonants degeminate** (nn/rr→n/r, gorffennaf→ɡɔrfɛnav). Residual (only 237/3426 are anglicized names — code-switching NOT the blocker): ⟨ae⟩ referee-contested, final-⟨e⟩→a (colloquial/lexical, net-negative as a rule), glide/quality edges. |
