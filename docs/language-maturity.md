...
| **cs** Czech | 94.8% (.92) | epitran 69.9% | 🟡 | Referee swapped epitran→**wikipron ces (human)**: 69.9→**94.8%** — epitran was systematically buggy (p→b before liquids: před→"bret"; no ě-palatalization), deflating where OURS is correct (now the demoted secondary). Real residual is **loanword-lexical** (the rule-based ceiling, as in espeak-ng-portable): di/ti/ni NON-palatalization in loans (stadion→stadɪjon, studie — natives correctly palatalize, tisíc→cɪsiːts) + loanword long í + foreign names. No independent source for the exception list (only wikipron = the referee), so it's not cleanly closable without gaming — a lexical ceiling, not espeak's cs_list (which is 395 letter-name entries, NOT the list the note claimed). |
