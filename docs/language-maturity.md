...
| **bn** Bengali | 37.5% (.34) | — | 🔵 | **First-pass native bring-up** (generic abugida engine + bengali.jsonc). Core phonology verified on goldens: dental t̪/d̪ vs retroflex ʈ/ɖ, sibilant merger শ ষ স→ʃ, ং→ŋ, inherent /ɔ/ + **vowel harmony** ɔ→o (করি→koɾi), **inherent-vowel deletion** (জল→d͡ʒɔl) with cluster/geminate retention as [o] (অংশ→ɔŋʃo), **phôla gemination** (jô/bô/mô: বিদ্যা→bid̪ːa, যুদ্ধ→d͡ʒud̪ʱːo). The 37.5% is **referee-noise-limited** — wikipron ben writes retroflex ট/ড as dental (a real contrast we keep) and varies word-by-word on literary final-[o] retention (~62% ceiling even for a mature engine per the sibling project); epitran is too crude to corroborate (no harmony/deletion). Remaining to reach a plateau: general conjunct clusters, [æ] realization of ⟨e⟩, medial inherent-vowel deletion, lexical final-vowel. See docs/bn_native_bringup_investigation.md. |
