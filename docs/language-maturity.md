...
| **bn** Bengali | 39.5% (.37) | — | 🔵 | **First-pass native bring-up** (generic abugida engine + bengali.jsonc). Core phonology verified on goldens: dental t̪/d̪ vs retroflex ʈ/ɖ, sibilant merger শ ষ স→ʃ, ং→ŋ, inherent /ɔ/ + **vowel harmony** ɔ→o (করি→koɾi), **inherent-vowel deletion** word-final (জল→d͡ʒɔl, cluster/geminate retained as [o]: অংশ→ɔŋʃo) AND **medial** (Ohala VCɔCV, shared with Hindi: আপনার→apnaɾ), **phôla gemination** (jô/bô/mô: বিদ্যা→bid̪ːa, যুদ্ধ→d͡ʒud̪ʱːo), **conjuncts** ক্ষ→kʰː / জ্ঞ→ɡː. The 39.5% is **referee-noise-limited** — wikipron ben writes retroflex ট/ড as dental (a real contrast we keep) and varies word-by-word on literary final-[o] retention (~62% ceiling even for a mature engine per the sibling project); epitran is too crude to corroborate (no harmony/deletion). Remaining to reach a plateau: [æ] realization of ⟨e⟩/্যা, lexical final-vowel, Ohala right-to-left mispicks on all-ɔ words (কলম). See docs/bn_native_bringup_investigation.md. |
