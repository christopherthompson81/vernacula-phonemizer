// The portable half of test/bavarian.test.ts — Bavarian (bar), Boarisch, Upper German (Austro-Bavarian),
// Latin script over the de-facto Bavarian-Wikipedia orthography (⟨å⟩ for the dark [ɔ], ⟨ä ö ü⟩, ⟨ß⟩).
//
...
{
    [Theory]
    // The FALLING diphthongs ⟨ia ua oa⟩→[iɐ̯ uɐ̯ oɐ̯] — Bavarian's hallmark — plus the closing ones.
    ...
    // R-vocalization, final-⟨a⟩ reduction, the ⟨gn⟩ coda coalescence.
    ...
    // Post-vocalic ⟨h⟩ is a silent length marker; the ich/ach dorsal split.
    ...
    // CARDINALS — units-FIRST like German but with the connector reduced from ⟨und⟩ to a bare linking ⟨-a-⟩.
    // Magnitudes are written OPEN: a closed *dreihundad would lose its ⟨h⟩ to the silent post-vocalic-h rule.
    [Theory]
    [InlineData(0, "null")]
    ...
    // ── TEXT NORMALIZATION ───────────────────────────────────────────────────────────────────────────
    // ⚠ Asserted on NormalizeBavarian — TEXT, not IPA — wherever the point is which WORDS the layer
    // chooses, because that is the decision under test and an IPA assertion buries it under the g2p.
    ...
    public void PeriodGroupedThousandsLoseTheDotBeforeItBecomesAPhraseBreak()
    {
        ...
        // unit noun that Bavarian, like German, CAPITALISES. The g2p lowercases before scanning, so the IPA
        // is byte-identical to the tidy spelling — asserted on the next line so the claim is checked.
        ...
        // exactly three digits, and this corpus's own dotted designation has two.
    ...
    // ⚠ THE SENTENCE-FINAL PERIODS, which must NOT be claimed — the check that matters here. The ISBN is
    // written out IN FULL rather than truncated: a two-link `3-8` is an ascending pair with no chain around
    // it, so the RANGE rule would read it as "3 bis 8". Shortening a fixture changes which rule owns it.
    ...
    // ⚠ AND THEY DECLINE WHERE THE WORD IS UNSOURCED rather than composing a wrong one. `ORDINAL` holds only
    // the five values read in bar.wikipedia prose, and the compositional route is refuted by this language's
    // own cardinals (`zeah` would give *zeaht, but the wiki writes `zehnte`).
    ...
    // masculine nominative article and the feminine dative one, and five of the eleven article-licensed
    // instances in the Bavarian subset take a governing preposition.
    ...
    // the `€` with it, because the tier needs `Milliardn` as a magnitude to hop.
    ...
    // class admits `-` inside a word run, so `fimf Uhr-nein Uhr` read *uɐ̯nɑɛ̯n*.
    [InlineData("5:00-9:00 Uhr", "fimf bis nein Uhr")]
    // ⚠ A THIRD FIELD IS NOT A CLOCK — these are triangle side-ratios, and the lookbehind has to exclude a
     ...
     *  and read *ɡ̥rɑd̥ʋiŋɡ̥l*, where before this rule existed they were two clean words. */
    [Fact]
    ...
    // a degree word directly after it, so a lookahead that cannot reach across the joiner reads "−1 bis MINUS
    // zwoa" — a span from POSITIVE one to minus two. Omitting a plus is lossless; omitting a minus INVERTS.
    ...
    // A general `(^|\s)[-−–](\d)` rule would have read these two as *minus 1990* and *minus 8*. The sign rule
    // declines both because no degree word follows; the RANGE rule then claims them, which is the reading
    // they should have — so the pair of rules disagreeing correctly is the thing under test here.
    ...
    // where the language writes `a` ("a hoiwe Milliardn", "A Quadratkilometa is a million moi").
    ...
    // ⚠ `Eiro`, NOT `Euro` — bar.wikipedia's own article reads "Da Eiro (amtli: Euro, Symboi: €)", so `Euro`
    // is the German/official form and would be a Standard German word in a Bavarian mouth.
    ...
    // The measure word FUSES onto the front — bar.wikipedia has a dedicated article: "A Quadratkilometa is a
    // million moi so grouß wia a Quadratmeta."
    ...
    [Theory]
    // THE DECIMAL COMMA. `Komma` scores 24 token hits on bar.wikipedia and every one is the VERB ("do komma
    // genau segn"); `insource:/[0-9] Komma [0-9]/` returns zero; bar.wiktionary has no entry. The comma stays
    // a pause rather than becoming a confidently wrong word in the highest-traffic slot.
    [InlineData("10,5 Millionan")]
    // `=` `<` `>` — 22 instances in the artifact and not one is arithmetic. They are the dialect wiki's own
    // morphology: a derivation arrow, a sound-change arrow, a glossing equals.
    [InlineData("da- (< der-)")]
    ...
    // bar's numeral arm is a bare `\d+`, so the TOKEN is the digit string and is threaded as `raw`.
