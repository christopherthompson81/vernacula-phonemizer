...
public static class Normalize
{
    ...
    *  worse here, ordinary Latin fragments inside the corpus's heavy English-language residue. */
    ...
    /** ⚠ THE SPELLER IS INJECTED, NOT IMPORTED, AND THAT IS A CYCLE-BREAKER. `numeralWords` is a non-negative
     *  integer → its Pashto numeral spelling, supplied by the engine so this file needs no import from
     *  `pashto.ts` — which would be a cycle, since the engine calls the normalizer. The `make…Normalizer(deps)`
     *  shape is the playbook's documented convention for exactly this. */
    public static Func<string, string> MakePashtoNormalizer(Func<double, string> numeralWords)
    {
        return input =>
        {
            ...
            {
                // ⚠ ONE HELPER FOR THE TS's TWO IDENTICAL REPLACERS — the sign-after and sign-before arms differ
                // only in their pattern, so the note below (written twice over there) stands once here.
