...
public static class Normalize
{
    ...
    // engine as the TS, so the identical source string is the only way the two ports can agree.
        JsRegex.Compile("(?<![\\p{Script=Greek}\\p{M}])(\\p{Script=Greek})([²³])?(?![\\p{Script=Greek}\\p{M}])", "gu");
    ...
    public static string NormalizeEnglish(string input)
    {
        ...
        {
            if (!GREEK_NAME.TryGetValue(m.Groups[1].Value, out var n)) return m.Value;
            var exp = m.Groups[2].Value;
            return exp.Length > 0 && GREEK_EXPONENT.TryGetValue(exp, out var e) ? n + e : n;
        });
