...
public static class Normalize
{
    ...
    public static string NormalizePolish(string input)
    {
        ...
        {
            var whole1 = s;
            s = Rewrite(s, re, m => KeepFinal(w, m.Value, m.Index, whole1));
        }
        ...
        var whole2 = s;
        s = Rewrite(s, KMH_NUM, m =>
        {
            var n = m.Groups[1].Value;
            return KeepFinal($"{n} {Counted(Js.Number(n), KMH)} na godzinę", m.Value, m.Index, whole2);
        });
        var whole2b = s;
        s = Rewrite(s, KMH_BARE, m => KeepFinal("kilometrów na godzinę", m.Value, m.Index, whole2b));
        s = Rewrite(s, MPH, m =>
            $"{m.Groups[1].Value} {Counted(Js.Number(m.Groups[1].Value), MILE)} na godzinę");
        s = Rewrite(s, MBPS, m => $"{m.Groups[1].Value} megabitów na sekundę");
        ...
        s = Rewrite(s, PAGE_S, m => $"{m.Groups[1].Value} strona ");
        s = Rewrite(s, NR, _ => "numer");
        ...
        s = Rewrite(s, VERSION_DOT, m => $"{m.Groups[1].Value} kropka ");
        ...
        s = Rewrite(s, RANGE, m => $"{m.Groups[1].Value} do ");
        ...
        s = Rewrite(s, PLUS, m => $"{m.Groups[1].Value}plus ");

        s = Rewrite(s, EQUALS, _ => " równa się ");
        ...
        s = Rewrite(s, LESS_THAN, _ => " mniejsze niż ");
        s = Rewrite(s, GREATER_THAN, _ => " większe niż ");
        s = Rewrite(s, DIVIDE, _ => " podzielić przez ");
