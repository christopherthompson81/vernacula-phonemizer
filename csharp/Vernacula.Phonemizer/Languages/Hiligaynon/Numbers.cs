/**
 * Hiligaynon cardinal number → words.
 * Ported from src/languages/hiligaynon/numbers.ts — see that file for the corpus evidence.
 */
using Vernacula.Phonemizer.Core;

namespace Vernacula.Phonemizer.Languages.Hiligaynon;

public sealed class HilNumbers
{
...
{
    private static HilNumbers N => HiligaynonPhonemizer.DEF.Numbers;
    ...
    public static string NumberToWords(double n, string? raw = null)
    {
        if (!(double.IsInteger(n) && Math.Abs(n) <= 9007199254740991d) || n < 0 || n >= 1e9)
            ...
            // which is worse than either sensible reading of it. Unreachable from `text()` (the token is
            // `\d+`), but `NumberToWords` is public and the TS answers it. Same finding as haw and gn.
            return string.Join(" ", Js.CodePoints(raw ?? Js.NumberToString(Math.Abs(n))).Select(d =>
                d.Length == 1 && d[0] >= '0' && d[0] <= '9' && d[0] - '0' < N.Units.Count
                    ? N.Units[d[0] - '0'] : d));
        ...
        var million = KaGroup(mil, N.Million); // isa ka milyon, duha ka milyon, …
