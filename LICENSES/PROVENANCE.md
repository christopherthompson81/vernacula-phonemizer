...
3. **`pashto/lexicon.tsv`** (14,021 rows) — the shipped Pashto short-vowel restoration lexicon,
   ...
   **GPL-3.0 fence**, same treatment as `wu/dict.tsv`; the engine reading it at runtime is not
   thereby GPL. Mixed-source and takes the most restrictive licence: 13,372 of 14,021 rows
   (95.4%) are reachable only from the GPL source, the rest also from wikipron/kaikki (CC-BY-SA),
