...
quantize_dynamic(fp32, f"{OUT}/bn-g2p-tagger.int8.onnx", weight_type=QuantType.QInt8)
os.remove(fp32)  # keep only the shipped int8 graph
print(f"exported: int8 {os.path.getsize(OUT+'/bn-g2p-tagger.int8.onnx')//1024}KB | chars {len(cv)} tags {len(lv)}")
