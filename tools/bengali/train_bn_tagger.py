...
rows = [l.rstrip("\n").split("\t")
        for l in open(SRC, encoding="utf8") if l.count("\t") == 1]
random.shuffle(rows)
nh = len(rows) // 5
hold = rows[:nh]
train = rows[nh:]
    ...
    for gi in range(len(G)):
        for pi in range(m + 1):
            for pl in cands(pi, m):
                prob[(G[gi], "".join(P[pi:pi + pl]))] = 1.0


def get(k): return prob.get(k, 1e-9)
        ...
        for gi in range(n):
            for pi in range(m + 1):
                if a[gi][pi] == 0:
                    continue
                for pl in cands(pi, m):
                    a[gi + 1][pi + pl] += a[gi][pi] * \
                        get((G[gi], "".join(P[pi:pi + pl])))
        Z = a[n][m]
        if Z <= 0:
            continue
        ...
        for gi in range(n):
            for pi in range(m + 1):
                ...
                for pl in cands(pi, m):
                    ...
                    if c > 0:
                        cnt[key] = cnt.get(key, 0.0) + c
    tot = {}
    for (g, p), c in cnt.items():
        tot[g] = tot.get(g, 0.0) + c
...
def viterbi_tags(G, P):
    n, m = len(G), len(P)
    best = [[(-1e18, None)] * (m + 1) for _ in range(n + 1)]
    best[0][0] = (0.0, None)
    for gi in range(n):
        for pi in range(m + 1):
            s = best[gi][pi][0]
            ...
            for pl in cands(pi, m):
                p = "".join(P[pi:pi + pl])
                ns = s + math.log(get((G[gi], p)) + 1e-12)
                if ns > best[gi + 1][pi + pl][0]:
                    best[gi + 1][pi + pl] = (ns, (pi, p))
    if best[n][m][1] is None:
        return None
    tags = [None] * n
    gi, pi = n, m
    while gi > 0:
...
print(f"# tagged {len(data)} | grapheme vocab={len(cv)} | tag vocab={len(lv)} | dev={dev}",
      file=sys.stderr, flush=True)


def genc(G): return [cv.get(c, 1) for c in G]
...
m.eval()
def oo(s): return "".join(c for c in s if c in "ɔo")
    ...
    for k in range(0, N, 512):
        ...
        for r, row in enumerate(b):
            ...
            for i, c in enumerate(G):
                ...
                if valid:
                    best = max(valid, key=lambda t: lo[r, i, t].item())
                else:
                    best = int(lo[r, i].argmax())
...
torch.save({"model": m.state_dict(), "cv": cv, "lv": lv,
           "char_mask": char_mask}, f"{OUTDIR}/bn_tagger.pt")
