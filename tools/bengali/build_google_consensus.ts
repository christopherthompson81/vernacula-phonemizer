...
const gold = new Set([
    ...load(
        `${HERE}/../referee-eval/referees/bn.gold-adjudicated.tsv`,
        1,
    ).keys(),
]);
...
for (const [w, gi] of G) {
    ...
    if (cf(gi) === cf(wi) && cf(gi) !== cf(e) && CS(gi) === CS(ec))
        out.push([w, gi.replace(/[ˈˌ]/gu, "")]);
}
out.sort(([a], [b]) => (a < b ? -1 : 1));
for (const [w, ip] of out) process.stdout.write(`${w}\t${ip}\n`);
process.stderr.write(
    `${out.length} Google∩wikipron consensus entries (vowel-only, non-gold, new)\n`,
);
