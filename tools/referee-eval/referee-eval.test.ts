...
describe("referee corroboration (segmental backbone vs the PRIMARY independent source)", () => {
    ...
    yue: 0.68, // epitran yue-Latn (INDEPENDENT, jyutping→IPA, crude) — 70.9% folded; adjudicated common-word Han→IPA gold (2nd) 100%. Han→Jyutping is rime-cantonese (121k, standard); jyutping→IPA authored in canonical IPA (aː/ɐ length, checked codas, 6 Chao tones). epitran MERGES aa/a length + drops checked vowels → it caps the number, not our quality
        ...
        my: 0.53, // wikipron mya_mymr broad (HUMAN, 8288) — 54.2% FOLDED; kaikki mya (2nd, HUMAN, 8107) 55.9%. Sino-Tibetan, the Mon-Burmese abugida — the HARDEST script/bring-up. Rime chart (vowel × coda), ⟨ွ⟩ labialisation (glide before -ng), minor-syllable ə, medial palatalisation (ကျ→t͡ɕ, ငြ→ɲ), voiceless ⟨ှ⟩ sonorants, stacked consonants. The backbone strips TONE, so it grades segments. TONES DONE (rule-based Chao, 99.6% mono / 97.9% per-syllable vs the tone-marked referees — tools/my-tone-eval.ts). 🟠 — two whole subsystems still DEFERRED (need a pronunciation lexicon): intervocalic voicing sandhi (k→ɡ/t→d after a vowel — biggest residual) + word segmentation (no spaces) + lexical rime variation (ည i~ɛ).
