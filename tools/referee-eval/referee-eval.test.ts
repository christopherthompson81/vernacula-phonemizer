...
describe("referee corroboration (segmental backbone vs the PRIMARY independent source)", () => {
    ...
    yue: 0.68, // epitran yue-Latn (INDEPENDENT, jyutping→IPA, crude) — 70.9% folded; adjudicated common-word Han→IPA gold (2nd) 100%. Han→Jyutping is rime-cantonese (121k, standard); jyutping→IPA authored in canonical IPA (aː/ɐ length, checked codas, 6 Chao tones). epitran MERGES aa/a length + drops checked vowels → it caps the number, not our quality
        ...
        my: 0.48, // wikipron mya_mymr broad (HUMAN, 8288) — 50.5% FOLDED; kaikki mya (2nd, HUMAN, 8107) 52.1%. Sino-Tibetan, the Mon-Burmese abugida — the HARDEST script/bring-up. Phase 1 SEGMENTAL: the rime chart (vowel × coda: ောင்→aʊɴ, ိုင்→aɪɴ, ိန்→eɪɴ, ုန்→oʊɴ, bare င்→ɪɴ), ⟨ွ⟩ labialisation, minor-syllable ə reduction, medial palatalisation (ကျ→t͡ɕ), voiceless ⟨ှ⟩ sonorants (မှ→m̥), stacked consonants. The backbone strips TONE (the referees deferred/folded), so it grades segments. 🟠 — deflated by the still-DEFERRED layers: the FOUR tones, intervocalic voicing sandhi (k→ɡ/t→d after a vowel — needs a lexicon), lexical rime variation (ည i~ɛ), and word segmentation (Burmese has no spaces).
