...
describe("referee corroboration (segmental backbone vs the PRIMARY independent source)", () => {
  ...
  const floors: Record<string, number> = {
    ...
    de: 0.62, // kaikki deu — measured 67.3% (wikipron deu 2nd: 67.2%); Run 10 added a compound-split retry after suffix strip (waldsterben→wald·sterben fires the st→ʃt seam). Residual DEFLATED by kaikki's proper-noun/loan bulk + stem-lexicon coverage gaps
