...
describe("referee corroboration (segmental backbone vs the PRIMARY independent source)", () => {
    ...
    fa: 0.40, // wikipron fas_arab broad (HUMAN, fully-voweled) — 42.9% FOLDED (short-vowel QUALITY a~e~o~i~u + long و=uː~oː / ی=iː~eː folded, all UNRECOVERABLE from the undiacritized abjad); adjudicated common-word gold (2nd) 88.9%. 🟠 scope-limited: the consonant + long-vowel SKELETON is correct (ʔ-initial, خوا→xʷaː, final ه→e, final clusters); short-vowel restoration is the deferred subsystem (like Urdu)
    ...
    yue: 0.68, // epitran yue-Latn (INDEPENDENT, jyutping→IPA, crude) — 70.9% folded; adjudicated common-word Han→IPA gold (2nd) 100%. Han→Jyutping is rime-cantonese (121k, standard); jyutping→IPA authored in canonical IPA (aː/ɐ length, checked codas, 6 Chao tones). epitran MERGES aa/a length + drops checked vowels → it caps the number, not our quality
        ...
        my: 0.66, // wikipron mya_mymr broad (HUMAN, 8288) — 69.0% FOLDED; kaikki mya (2nd, HUMAN, 8107) 71.4% (partly self-referential — the voicing lexicon is mined from kaikki; wikipron 69.0% is the honest independent signal). Sino-Tibetan, Mon-Burmese abugida — the HARDEST script/bring-up. Rime chart (vowel × coda), ⟨ွ⟩ labialisation (glide before -ng), minor-syllable ə, medial palatalisation (ကျ→t͡ɕ, ငြ→ɲ), voiceless ⟨ှ⟩ sonorants, stacked consonants. TONES done (rule-based Chao, 99.6% mono — tools/my-tone-eval.ts). VOICING sandhi done (per-word voicing-lexicon.tsv mined from kaikki, corroborated on wikipron +1210/−4). 🟠 — ONE subsystem still DEFERRED: word segmentation (Burmese is spaceless; the eval is per-word so 69% is a real word-level signal).
