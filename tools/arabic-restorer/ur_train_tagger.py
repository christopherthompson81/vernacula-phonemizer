#!/usr/bin/env python3
"""Urdu STRUCTURAL tagger — port of the fa tagger (tools/fa-restoration/train_tagger.py) to the Hindi-phonology
...
SILVER = f"{HERE}/tools/arabic-restorer/silver.hindiurdu.tsv"
...
EXTRA = f"{HERE}/tools/arabic-restorer/ur_extra_pool.tsv"
def load_pool():
    pool=[]; seen=set()
    files=[SILVER]
...
torch.save({"model":m.state_dict(),"cv":cv,"lv":lv,"char_mask":char_mask.cpu()}, f"{HERE}/tools/arabic-restorer/ur_tagger.pt")
