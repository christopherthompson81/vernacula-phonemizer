...
def encode(rows, src, tag2id):
    """Slots whose gold tag carries the INHERENT-vowel marker ᵊ are labelled -100 = IGNORED. Devanagari's
    unmarked consonant means "default vowel", not "ə" — training on it teaches a systematic ə bias
    (measured 61:12 against the independent mine). Masking keeps only trustworthy labels: explicit vowel
    signs and virama."""
    X, Y = [], []
    for _, letters, tg in rows:
        X.append([src.get(c, 1) for c in letters])
        Y.append([-100 if "ᵊ" in t else tag2id[t] for t in tg])
    return X, Y
...
def evaluate(model, rows, X, Y, tags, char_tags, src, dev):
    ...
    with torch.no_grad():
        for k, (w, letters, gold) in enumerate(rows):
            ...
            word_n += 1
            if [p for p, g in zip(pred, gold) if "ᵊ" not in g] == [g for g in gold if "ᵊ" not in g]:
                word_hit += 1
            for i, g in enumerate(gold):
                if "ᵊ" in g or not is_decision(g):
                    continue
                slot_n += 1
                if pred[i] == g:
                    slot_hit += 1
