...
# env-configurable so the same loader serves the segmental perceptron prototype AND the stress-included BiLSTM:
#   NB_LEX          — training tsv (word\tIPA[\tvariant…]); default the segmental dump
#   NB_KEEP_STRESS  — keep ˈ/ˌ in the IPA (the BiLSTM predicts stress POSITION; the prototype strips it)
#   NB_SUBSAMPLE    — deterministic md5-bucket cap for a fast pass (0 = full NST, ~630k)
LEX = os.environ.get("NB_LEX", "/tmp/nb_train.tsv")
KEEP_STRESS = bool(os.environ.get("NB_KEEP_STRESS"))
SUBSAMPLE = int(os.environ.get("NB_SUBSAMPLE", "150000"))
OUT = "/tmp/nb_holdout.tsv"

VOWELS = set("aeiouyæøåéèêëàâôü")
...
def load():
    ...
    for line in open(LEX, encoding="utf-8"):
        ...
        if not line or "\t" not in line:
            continue
        w, ipa = line.split("\t", 1)
        ipa = ipa.split("\t", 1)[0]  # first (shortest/canonical) variant if the tsv carries alternates
        if not KEEP_STRESS:
    ...
    if SUBSAMPLE and len(rows) > SUBSAMPLE:
        rows = sorted(rows, key=lambda r: hashlib.md5(('samp:'+r[0]).encode()).hexdigest())[:SUBSAMPLE]
    return rows
...
def split(rows):
    ...
    def bucket(w):
        return int(hashlib.md5(("nb:" + w).encode("utf-8")).hexdigest(), 16) % 10
    tr, te = [], []
    for w, p in rows:
