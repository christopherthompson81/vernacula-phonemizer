...
# live beside each rider's g2p (data/languages/<lang>/lexicon.tsv). Strips to 2 columns (skeleton ⇥ vocalized),
...
for L in ur fa ps pa; do
    ...
    awk -F'\t' '$1!=$3{print $1"\t"$3}' "$src" | LC_ALL=C sort | awk -F'\t' '!seen[$1]++' > "$body"
    {
        ...
        if [ "$L" = "ps" ]; then
            ...
            echo "# See data/languages/pashto/lexicon.PROVENANCE.md and LICENSES/PROVENANCE.md §4."
        else
