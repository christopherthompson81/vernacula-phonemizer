...
describe("english normalization: alphanumeric codes, money, signs, numeric dates", () => {
    ...
    test("money reads as currency-plus-cents, not as a decimal", () => {
        ...
        expect(phonemize("it cost $5.50", "en")).toBe("ɪt kʰˈɑːst fˈaᶦv dˈɑːlɚz ənd fˈɪfti sˈɛnts");
        expect(phonemize("$1.99", "en")).toBe("wˈʌn dˈɑːlɚ ənd nˈaᶦnti nˈaᶦn sˈɛnts"); // singular at 1
        expect(phonemize("£2.50 each", "en")).toBe("tʰˈuː pʰˈaᶷndz ənd fˈɪfti pʰˈɛns ˈiːt͡ʃ");
    });
    ...
    test("a magnitude abbreviation glued to a money figure is million/billion/thousand", () => {
        ...
        // denies it, so `$2.30bn` never becomes "2 dollars and 30 cents".
        expect(normalizeEnglish("$5.50")).toBe("5 dollars and 50 cents");
