...
describe("referee corroboration — English (split out for wall time)", () => {
    const floors: Record<string, number> = {
        en: 0.47, // wikipron eng_us. ⚠ RAISED FROM 0.35, WHICH HAD STOPPED BEING A GATE: #1327 and #1328 moved the INSTRUMENT (not the engine) from 40.9% to 56.1% by adding three missing convention folds, scoring the neural tier that actually ships, and dropping 488 RP rows from a referee labelled GenAm — so 0.35 sat 20pp below the measurement and would not have caught a regression of any size. Sampled now 54.9% (1118/2035). ⚠ 0.47 IS BELOW THE DEGRADED-PATH SCORE ON PURPOSE: with onnxruntime unavailable `phonemizeEnNeural` falls back to the sync engine, which measures 50.0% on the full referee — so this floor survives an ONNX-less environment, and a result NEAR 50% rather than 55% is the signature of that fallback rather than of a regression. Previously documented as "measured 41.8%", which was the pre-#1327 instrument. #1282's ᵻ→ɪ fold is worth +1.8pp of it: the referee has no ᵻ, so leaving it unfolded scored the house weak-vowel convention as ALWAYS-WRONG across 5,580 lexicon rows and made a CORRECT change (#1275's -es plural) cost 7 words. ⚠ THE FOLD HIDES WHETHER ᵻ IS PLACED CORRECTLY — that axis lives in tools/english/en_weak_vowel_survey.mts; run it when isBarredI changes.
