...
// SHAPE had to be read first. lb's arm is a bare `\\d+`, so the token IS the digit string.
//
...
describe("a numeral past 2^53 reads the digits the writer typed", () => {
    const words = (s: string): string[] => phonemize(s, "lb").trim().split(" ");
    ...
    test("the low digits are the token's, not the double's", () => {
        ...
        expect(words("9007199254740993").slice(-1)[0]).toBe("drˈæi"); // …993, was its neighbour's …992
    });
