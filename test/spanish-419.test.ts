 ...
 * is NOT the story: it MATCHES the es parent's, because the residual is entirely inherited
 * coda-obstruent-voicing allophony shared with es. This gold verifies the two mergers are exact on
...
describe("es-419 Roman-numeral policy — centuries cardinal, prenominal events ordinal", () => {
    ...
    test("a century stays a CARDINAL (the century noun is not a trigger)", () => {
        ...
        expect(phonemize("siglo xix", "es-419")).toBe('sˈiɣlo ðjesinwˈeβe');
    });
...
// TEXT NORMALIZATION (spanish/normalize.ts). Modelled on English
...
describe("spanish normalization", () => {
    test("abbreviations, with EE.UU. claimed first", () => {
        ...
        expect(phonemize("el Dr. García", "es-419")).toBe("el doktˈoɾ ɣaɾsˈia"); // was the cluster [dɾ] + a pause
        ...
        expect(phonemize("356 a. C.", "es-419")).toBe("tɾessjˈentos sinkwˈenta i sˈeᶦs ˈantes ðe kɾˈisto");
        expect(phonemize("el año 33 d.C.", "es-419")).toBe("el ˈaɲo tɾˈeᶦnta i tɾˈes ðespwˈes ðe kɾˈisto");
        // Read as letter names in Spanish, and the interior dots were two more phrase breaks.
        expect(phonemize("a las 10:08 p. m.", "es-419")).toBe("a las ðjˈes ˈot͡ʃo pˈe ˈeme");
    ...
    test("ordinal indicators no longer leak, and ° is not one of them", () => {
        ...
        expect(phonemize("el 1º de mayo", "es-419")).toBe("el pɾimˈeɾo ðe mˈaʝo");
        expect(phonemize("la 1ª vez", "es-419")).toBe("la pɾimˈeɾa βˈes"); // feminine
        ...
        expect(phonemize("20 °C", "es-419")).toBe("bˈeᶦnte ɣɾˈaðos sˈelsjus");
        expect(phonemize("35°", "es-419")).toBe("tɾˈeᶦnta i sˈinko ɣɾˈaðos");
    });

    test("initialisms: unpronounceable letter strings are spelled out", () => {
        expect(phonemize("un CD", "es-419")).toBe("un sˈe ðe"); // was [kð]
        expect(phonemize("el ADN", "es-419")).toBe("el a ðe ˈene"); // was [aðn]
    ...
    test("grouping, units, fractions and signs", () => {
        ...
        expect(phonemize("120 km/h", "es-419")).toBe("sjˈento βˈeᶦnte kilˈometɾos poɾ ˈoɾa"); // /h was dropped
        expect(phonemize("83 m", "es-419")).toBe("ot͡ʃˈenta i tɾˈes mˈetɾos"); // was the letter name
        expect(phonemize("90 °F", "es-419")).toBe("noβˈenta ɣɾˈaðos faɾenˈeᶦt");
        ...
        expect(phonemize("+3 grados", "es-419")).toBe("mˈas tɾˈes ɣɾˈaðos");
        expect(phonemize("-5 grados", "es-419")).toBe("mˈenos sˈinko ɣɾˈaðos");
    });
    ...
    test("the first of the month is the one variety-specific rule", () => {
        ...
        expect(phonemize("el 1 de enero", "es-419")).toBe("el pɾimˈeɾo ðe enˈeɾo");
        expect(phonemize("el 1 de enero", "es")).toBe("el ˈuno ðe enˈeɾo");
        expect(phonemize("el 17 de septiembre", "es-419")).toBe("el djesisjˈete ðe septjˈembɾe");
    });
});
