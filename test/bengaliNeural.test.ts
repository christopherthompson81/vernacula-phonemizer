...
// model present. See data/languages/bengali/bn-g2p-tagger.PROVENANCE.md.
...
describe("bengali neural OOV tagger", () => {
    ...
    describe.skipIf(!haveModel)("with the ONNX model present", () => {
        ...
        test("OOV word: tagger raises the inherent vowel the rule engine gets wrong", async () => {
            ...
            // ⚠ The danda must arrive as a CANONICAL pause mark. If clausePunctuation maps a mark to itself,
            // the raw danda reaches the output padded with spaces instead.
        ...
        test("mixed text: numbers + punctuation unchanged, only the OOV word retagged", async () => {
            // 1999 = এক হাজার নয় শত নিরানব্বই. ⚠ Two data faults produce "নয় একশো নয় নব্বই" here
            // ("nine one-hundred, nine ninety"): missing fused 21-99 forms, and a `hundred` of "একশো",
            // which already contains its own এক.
